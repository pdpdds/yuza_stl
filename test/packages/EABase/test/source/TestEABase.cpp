...
int TestEABase()
{
	...
	{
		static_assert(INTPTR_MIN  == std::numeric_limits<intptr_t>::min(),  "INTPTR_MIN failure");
		static_assert(INTPTR_MAX  == std::numeric_limits<intptr_t>::max(),  "INTPTR_MAX failure");
	  //static_assert(UINTPTR_MIN == std::numeric_limits<uintptr_t>::min(), "UINTPTR_MIN failure"); // not specified by the standard
		static_assert(UINTPTR_MAX == std::numeric_limits<uintptr_t>::max(), "UINTPTR_MAX failure");
		static_assert(INTMAX_MIN  == std::numeric_limits<intmax_t>::min(),  "INTMAX_MIN failure");
		static_assert(INTMAX_MAX  == std::numeric_limits<intmax_t>::max(),  "INTMAX_MAX failure");
	  //static_assert(UINTMAX_MIN == std::numeric_limits<uintmax_t>::MIN(), "UINTMAX_MIN failure"); // not specified by the standard
		static_assert(UINTMAX_MAX == std::numeric_limits<uintmax_t>::max(), "UINTMAX_MAX failure");
	}
...
bool TestNU()
{
	...
	template<typename T> 
	using VectorAlias = std::vector<T, EASTLAllocatorType>;
#endif
...
int TestEACompiler()
{
	...
	#if !defined(EA_COMPILER_NO_INITIALIZER_LISTS)
	{
		...
		//#include <initializer_list>
		//std::vector<std::string, int> anim = { {"bear", 4}, {"cassowary", 2}, {"tiger", 7} };
	...
	{
		struct LocalStruct{};  
		std::fixed_vector<LocalStruct, 2, false> localStructArray;
...
static void TestInt128T(MakeInt128T MakeInt128, const char* errorMsg, int& nErrorCount) 
{
	...
	// default ctor
	{
		...
		static_assert(std::is_trivially_default_constructible_v<Int128T>, "128-bit integer failure");
	}
