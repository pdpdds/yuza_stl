...
int ExecuteShellCommand(const char* pCommand)
{
	...
	EASTDC_API bool SearchEnvPathWithMode(const char* pathListVar, const char* fileName, int mode, std::string8* fullPath)
	{
...
EASTDC_API bool SearchEnvironmentPath(const char* pFileName, char* pPath, const char* pEnvironmentVar)
{
		...
		std::string8 path8(EASTLAllocatorType(UTFFOUNDATION_ALLOC_PREFIX "EAProcess"));
		bool success;
