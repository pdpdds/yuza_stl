...
#include <new>
	...
	{
		typedef std::map<const wchar_t*, ea_char16_str> WChar16Map; // Map of wchar_t* to char16_t* 

		static WChar16Map sTable; // Not thread safe.
		ea_char16_str s = ea_char16_str(pText);
		std::pair<WChar16Map::iterator, bool> result = sTable.insert(std::make_pair(pText, s));
	...
	{
		typedef std::map<const wchar_t*, ea_char32_str> WChar32Map; // Map of wchar_t* to char32_t* 

		static WChar32Map sTable; // Not thread safe.
		ea_char32_str s = ea_char32_str(pText);
		std::pair<WChar32Map::iterator, bool> result = sTable.insert(std::make_pair(pText, s));
		return result.first->second.mpData;
	}
