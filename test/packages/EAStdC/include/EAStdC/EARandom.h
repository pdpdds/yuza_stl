...
namespace StdC
{
	...
	///     #include <eastl/algorithm>   // or #include <algorithm> to use std STL.
	///     
	///     EA::StdC::Random rand(someSeedValue);                       // We can just use EA::StdC::Random directly because 
	///     std::generate(myVector.begin(), myVector.end(), rand);    // it has an operator() that returns uint32_t.
	///
	...
	///     std::generate(myVector.begin(), myVector.end(), rand);
	/////////////////////////////////////////////////////////////////////////////////////
