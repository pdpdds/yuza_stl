...
int TestSList()
{
	...
	// slist(this_type&& x);
	{
		...
		slist<int> list2(std::move(list1));
	...
	// const_iterator cend() const EA_NOEXCEPT;
	{
		...
		VERIFY(i == std::end(list1));
		VERIFY(ci == std::end(list1));
		VERIFY(ci2 == std::end(list1));
	}
	...
	// const_reference front() const;
	{
		...
		VERIFY(list1.begin() == std::begin(list1));
	...
	// void     insert(const_iterator position, size_type n, const value_type& value);
	{
		...
		auto insert_iter = std::begin(list1);
	...
	// void insert(const_iterator position, InputIterator first, InputIterator last);
	{
		...
		list1.insert(std::end(list1), std::begin(list2), std::end(list2)); // [42,42,42,...,42, | 24,24,24,24...]
		VERIFY(!list1.empty());
		...
		std::advance(boundary_iter, 100); // move to insertation point
	...
	// iterator insert_after(const_iterator position, std::initializer_list<value_type> ilist);
	{
		...
		VERIFY(std::count_if(list1.begin(), list1.end(), [](int i) { return i == 42; }) == 10);
		VERIFY(list1.validate());
	...
	{
		typedef std::slist<int, fixed_allocator> SIntList;
	...
	// void splice(const_iterator position, this_type&& x, const_iterator first, const_iterator last);
	{
		...
		}
		{
			...
			list1.splice(list1.begin(), std::move(list2), b, e);
	...
	{
		{
			...
			list1.splice_after(list1.begin(), std::move(list2));
			VERIFY(list1 == slist<int>({0,4,5,6,7,1,2,3}));
		}
		{
			slist<int> list1 = {0,1,2,3};
			...
			list1.splice_after(list1.begin(), std::move(list2), list2.begin());
		...
		}
		{
			...
			list1.splice_after(list1.begin(), std::move(list2), b, e);
	...
	// void sort();
	{
		...
		VERIFY(!std::is_sorted(std::begin(list1), std::end(list1)));
		VERIFY(list1.validate());
	...
	// void sort(Compare compare);
	{
		...
		slist<int> list1 = {0, 1, 2, 2, 2, 3, 4, 5, 6, 7, 8, 9, 9, 8, 7, 6, 5, 4, 3, 2, 2, 2, 1, 0};
		VERIFY(!std::is_sorted(std::begin(list1), std::end(list1), compare));
		list1.sort(compare);
		VERIFY(std::is_sorted(std::begin(list1), std::end(list1), compare));
	}
	...
	{ // Test empty base-class optimization
		...
		typedef std::slist<int, std::dummy_allocator> list1;
		typedef std::slist<int, UnemptyDummyAllocator> list2;
	...
	{ // Test erase / erase_if
...

		{
			...
			std::erase_if(l, [](auto e) { return e % 2 == 0; });
			VERIFY((l == slist<int>{1, 3, 5, 7, 9}));

			std::erase_if(l, [](auto e) { return e == 5; });
			VERIFY((l == slist<int>{1, 3, 7, 9}));

			std::erase_if(l, [](auto e) { return e % 3 == 0; });
