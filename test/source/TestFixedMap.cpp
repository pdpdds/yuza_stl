...
template class std::fixed_map     <int,        float,      1>;
template class std::fixed_multimap<float,      int,        1>;
template class std::fixed_map     <int,        TestObject, 1>;
template class std::fixed_multimap<TestObject, int,        1>;

template class std::fixed_map     <int,        float,      1, true, std::less<int>,        MallocAllocator>;
template class std::fixed_multimap<float,      int,        1, true, std::less<float>,      MallocAllocator>;
template class std::fixed_map     <int,        TestObject, 1, true, std::less<int>,        MallocAllocator>;
template class std::fixed_multimap<TestObject, int,        1, true, std::less<TestObject>, MallocAllocator>;
...
typedef std::fixed_map<int, int, kContainerSize> VM1;
typedef std::fixed_map<TestObject, TestObject, kContainerSize> VM4;
typedef std::fixed_multimap<int, int, kContainerSize> VMM1;
typedef std::fixed_multimap<TestObject, TestObject, kContainerSize> VMM4;
...
int TestFixedMap()
{
...

	{
		...
		nErrorCount += TestMapCpp11<std::fixed_map<int, TestObject, 32> >();

		nErrorCount += TestMultimapCpp11<std::fixed_multimap<int, TestObject, 32> >();

		nErrorCount += TestMapCpp11NonCopyable<std::fixed_map<int, NonCopyable, 32>>();
	}

	{
		// C++17 try_emplace and related functionality
		nErrorCount += TestMapCpp17<std::fixed_map<int, TestObject, 32>>();
	}
	...
	{   // Regression of bug report by Eric Turmel, May 20, 2008
		typedef std::fixed_map<int, TestObject, 37, false> FixedMap;
