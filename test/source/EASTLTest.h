...
// This function is an analog of the std::use_self and use_first functions.
...
/// test_use_self
///
/// Intentionally avoiding a dependency on std::use_self.
///
template <typename T>
...
///
/// Example usage:
///     typedef demoted_iterator<int*, std::bidirectional_iterator_tag>         PointerAsBidirectionalIterator;
///     typedef demoted_iterator<MyVector::iterator, std::forward_iterator_tag> VectorIteratorAsForwardIterator;
///
/// Example usage:
///     IntVector v;
...
// CustomAllocator
//
// Implements an allocator that works just like std::allocator but is defined
...
/// UnequalAllocator
///
/// Acts the same as std::allocator, but always compares as unequal to an 
/// instance of itself.
///
...
///
class CountingAllocator : public std::allocator
{
public:
	using base_type = std::allocator;
...
// 
//         std::list<int, InstanceAllocator> list0(1, ia0);
//         std::list<int, InstanceAllocator> list1(1, ia1);
// 
...
public:
	...
	{ // +1 so that we always have space to write mInstanceId.
		...
		std::fill(p8, p8 + kMultiplier, 0xff);
...
{
	std::allocator* mPrevAllocator = nullptr;

	AutoDefaultAllocator(std::allocator* nextAllocator) { mPrevAllocator = SetDefaultAllocator(nextAllocator); }
