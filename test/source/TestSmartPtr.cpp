...
#include <EASTL/linked_array>
#include <EASTL/linked_ptr.h>
#include <EASTL/safe_ptr.h>
#include <EASTL/scoped_array>
#include <EASTL/scoped_ptr.h>
#include <EASTL/shared_array>
...
namespace SmartPtrTest
{
	...
	/// This is used for tests involving shared_ptr.
	/// 
	class ACLS : public std::enable_shared_from_this<ACLS>
	{
	public:
		static int mnCount;
		int a;
	...
	struct CheckUPtrEmptyInDestructor
	{
		...
		std::unique_ptr<CheckUPtrEmptyInDestructor>* mpUPtr{};
	...
	struct CheckUPtrArrayEmptyInDestructor
	{
		...
		std::unique_ptr<CheckUPtrArrayEmptyInDestructor[]>* mpUPtr{};
		static bool mCheckUPtrEmpty;
	};
...
static int Test_unique_ptr()
{
...

	{
		...
		*pT1 = 3;
		...
		// unique_ptr(pointer pValue, typename std::remove_reference<Deleter>::type&& deleter) noexcept
		unique_ptr<A, CustomDeleter> pT7(new A(18), CustomDeleter());
		EATEST_VERIFY(pT7->mc == 18);

		// unique_ptr(this_type&& x) noexcept
		unique_ptr<A, CustomDeleter> pT8(std::move(pT7));
		EATEST_VERIFY(pT8->mc == 18);

		// unique_ptr(unique_ptr<U, E>&& u, ...)
		unique_ptr<A, default_delete<A> > pT9(std::move(pT2));
		...
		//pTVoid.operator=<int, CustomDeleter>(std::move(pTInt));  // This doesn't work because CustomDeleter doesn't know how to delete void*. Need to rework this test.
		...
		// ensure a unique_ptr containing nullptr doesn't call the deleter when its destroyed.
		{
			...
			using local_unique_ptr = std::unique_ptr<int, LocalDeleter>;
...

	{
			...
			// Test EA::Allocator::EASTLICoreDeleter usage within std::shared_ptr.
			// http://en.cppreference.com/w/cpp/memory/shared_ptr/shared_ptr

			// Consider the following for standards compliance.
			// std::shared_ptr<A, EASTLCoreDeleterAdapter> foo(pA, EASTLCoreDeleterAdapter());
			...
			{            
				A* pA = new (pMem) A();
				std::shared_ptr<A> foo(pA, EASTLCoreDeleterAdapter());  // Not standards complaint code.  Update EASTL implementation to provide the type of the deleter.
			}
			EATEST_VERIFY(gEASTLTest_AllocationCount == cacheAllocationCount);
			EATEST_VERIFY(A::mCount == 0);
		#endif
...

	{
		...
		// unique_ptr(this_type&& x) noexcept
		unique_ptr<A[], CustomArrayDeleter> pT8(std::move(pT7));
		...
		unique_ptr<NamedClass> p = std::make_unique<NamedClass>("test", "test2");
		EATEST_VERIFY(EA::StdC::Strcmp(p->mpName, "test") == 0 && EA::StdC::Strcmp(p->mpName2, "test2") == 0);

		unique_ptr<NamedClass[]> pArray = std::make_unique<NamedClass[]>(4);
		...
		#ifdef EASTL_TEST_DISABLED_PENDING_SUPPORT
		{
			...
			unique_ptr<AlignedStruct> pAlignedStruct = std::make_unique<AlignedStruct>();
		...
		//unique_ptr<NamedClass[4]> p2Array4 = std::make_unique<NamedClass[4]>();
	...
	// validate unique_ptr's compressed_pair implementation is working. 
	{
		...
		// std::compressed_pair implementation will remove that deleter data member from the unique_ptr.
		{
...
static int Test_shared_ptr()
{
	...
	// Name test.
		...
		//std::string sTypeName = GetTypeName<typename std::unique_ptr<int>::pointer>();
		//EA::UnitTest::Report("type name of (typename shared_ptr<int>::pointer): %s", sTypeName.c_str());

		//sTypeName = GetTypeName<typename std::common_type<int*, int*>::type>();
		//EA::UnitTest::Report("type name of (typename std::common_type<int*, int*>::type): %s", sTypeName.c_str());
	#endif
	...
	// operator=(unique_ptr<U, Deleter> && uniquePtr)
	{
		...
		// test the state of the shared_ptr::operator= return
		{
			...
			shared_ptr<A> operatorReturn = (rT1 = std::move(rT2));
	...
	{ // Test static_pointer_cast, etc.
		...
		EATEST_VERIFY(pCC == pGCC);
			...
			std::shared_ptr<const void> pVoidPtr = shared_ptr<ParentClass>(new ParentClass);
...

	{
		// Exception handling tests
			...
			try {
				...
				shared_ptr<A> pSharedA(pWeakA);   // This should throw std::bad_weak_ptr
				EATEST_VERIFY(false);
			}
			...
			try {
				A::mCount = 0;
				pA0 = std::allocate_shared<A, ThrowingAllocator<true> >(throwingAllocator, 'a');
				EATEST_VERIFY(false);
			}
			catch(std::bad_alloc&)
			{
	...
	struct SharedPtrTestThread : public EA::Thread::IRunnable
	{
		...
		int                             mnErrorCount;
		std::shared_ptr<TestObject>*  mpSPTO;
		std::weak_ptr<TestObject>*    mpWPTO;
		...
		intptr_t Run(void*)
		{
			...
			while(mbShouldContinue)
			{
				...
				std::shared_ptr<TestObject> temp(mpWPTO->lock());
				EATEST_VERIFY(temp->mX == 99);

				std::shared_ptr<TestObject> spTO2(*mpSPTO);
				EATEST_VERIFY(spTO2->mX == 99);
				EATEST_VERIFY(spTO2.use_count() >= 2);

				std::weak_ptr<TestObject> wpTO2(spTO2);
				temp = mpWPTO->lock();
				EATEST_VERIFY(temp->mX == 99);
...
static int Test_linked_ptr()
{
	...
	{   // Verify that subclasses are usable.
		...
		std::linked_ptr<DerivedMockObject> pDMO(new DerivedMockObject(&bAlloc));
		std::linked_ptr<MockObject> a1(pDMO);
		std::linked_ptr<MockObject> a2;
...
static int Test_intrusive_ptr()
{
	...
	{ 
		// Test move-ctor
		{
			...
			{
				intrusive_ptr<RefCountTest> ip2(std::move(ip1));
...
struct RandomLifetimeObject : public std::safe_object
{
...
static int Test_safe_ptr()
{
	...
	{   // non-const RandomLifetimeObject
		...
		std::safe_ptr<RandomLifetimeObject> pSafePtr(pObject);

		std::safe_ptr<RandomLifetimeObject> pSafePtrCopy1 = pSafePtr;
		std::safe_ptr<RandomLifetimeObject> pSafePtrCopy2(pSafePtr);

		pSafePtr->DoSomething();

		std::safe_ptr<RandomLifetimeObject>* pSafePtrCopy3 = new std::safe_ptr<RandomLifetimeObject>(pSafePtr);
		std::safe_ptr<RandomLifetimeObject>* pSafePtrCopy4 = new std::safe_ptr<RandomLifetimeObject>(pSafePtr);
	...
	{   // const RandomLifetimeObject
		RandomLifetimeObject* pObject = new RandomLifetimeObject;
		std::safe_ptr<const RandomLifetimeObject> pSafePtr(pObject);

		std::safe_ptr<const RandomLifetimeObject> pSafePtrCopy1(pSafePtr);
		std::safe_ptr<const RandomLifetimeObject> pSafePtrCopy2 = pSafePtr;

		pSafePtr->DoSomething();

		std::safe_ptr<const RandomLifetimeObject>* pSafePtrCopy3 = new std::safe_ptr<const RandomLifetimeObject>(pSafePtr);
		std::safe_ptr<const RandomLifetimeObject>* pSafePtrCopy4 = new std::safe_ptr<const RandomLifetimeObject>(pSafePtr);
		EATEST_VERIFY(pSafePtrCopy3->get() == pObject);
		EATEST_VERIFY(pSafePtrCopy4->get() == pObject);
		delete pSafePtrCopy3;
		delete pSafePtrCopy4;
