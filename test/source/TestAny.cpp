...
int TestAny()
{
	...
	// NOTE(rparolin): Ensure 'any' is at least the size of an std::string and an std::vector to prevent heap
	// allocation of handle objects (objects that point to a heap allocation).  This will reduce memory pressure since
	// std::string will be a commonly used type.  We could also test with a vector.
	{
		static_assert(sizeof(std::string) <= sizeof(std::any), "ensure that 'any' has enough local memory to store a string");
		static_assert(sizeof(std::vector<int>) <= sizeof(std::any), "ensure that 'any' has enough local memory to store a vector");
	}
	...
	// user reported regression that std::any constructor was not decaying the deduced type correctly.
	{
		float f = 42.f;
		std::any a(f);
	...
	//testing unsafe operations
	{
		...
		a = 2;
		int *j = (int*)std::unsafe_any_cast<void>(&a);
		VERIFY((*j) == 2);

		const std::any b = 3;
		const void * p = std::unsafe_any_cast<void>(&b);
