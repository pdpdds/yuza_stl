...
// TestObjectNegate
//
struct TestObjectNegate : public std::unary_function<TestObject, TestObject>
{
...
static int TestMinMax()
{
	...
	{
		// NOTE(rparolin): This compiles but it should not.  We provide explicit std::max overloads for float, double,
		...
		// NOTE(rparolin): This will not compile because we lack explicit std::max overloads for 'int'.
		// {
		//	 struct Foo
		//	 {
...

	{
		...
		static_assert(std::is_enum<TestMinMaxEnum>::value, "is_enum failure");
		EATEST_VERIFY(std::min(teX, teY) == teX);

		// Make sure pointers work
		TestObject testObjectArray[2];
		EATEST_VERIFY(std::min(&testObjectArray[0], &testObjectArray[1]) == &testObjectArray[0]);
		...
		int32_t result = std::max_alt<int32_t>(0, value); // warning C4347: behavior change: 'const T &std::max_alt<int32_t>(const T &,const T &)' is called instead of 'int std::max_alt(int,int)'
		...
		uint32_t value2a  = 17;
		uint32_t value2b  = 2;
		uint32_t result2 = std::min_alt<uint32_t>(value2a - value2b, 4); // error C2666: 'std::min_alt' : 12 overloads have similar conversions
		...
		// uint32_t result3 = std::min_alt<uint32_t>(value3, 4); // error C2664: 'const T &std::min_alt<unsigned int>(const T &,const T &)' : cannot convert parameter 1 from 'volatile uint32_t' to 'const unsigned int &'
...

	{
		...
		std::pair<int*, int*> result = std::minmax_element(intArray, intArray + 6);
		...
		//     warning C4413: 'std::pair<T1,T2>::first' : reference member is initialized to a temporary that doesn't persist after the constructor exits.
		...
		#if !defined(EA_COMPILER_NO_INITIALIZER_LISTS)
			std::pair<int, int> result3 = std::minmax({3, 2});
...
static int TestClamp()
{
	...
	EATEST_VERIFY(std::clamp(42, 1, 100) == 42);
	EATEST_VERIFY(std::clamp(-42, 1, 100) == 1);
	EATEST_VERIFY(std::clamp(420, 1, 100) == 100);
	EATEST_VERIFY(std::clamp(1, 1, 100) == 1);
	EATEST_VERIFY(std::clamp(100, 1, 100) == 100);

	EATEST_VERIFY(std::clamp(42.f, 1.f, 100.f, less<float>()) == 42.f);
	EATEST_VERIFY(std::clamp(-42.f, 1.f, 100.f, less<float>()) == 1.f);
	EATEST_VERIFY(std::clamp(420.f, 1.f, 100.f, less<float>()) == 100.f);
	EATEST_VERIFY(std::clamp(1.f, 1.f, 100.f, less<float>()) == 1.f);
	EATEST_VERIFY(std::clamp(100.f, 1.f, 100.f, less<float>()) == 100.f);

	EATEST_VERIFY(std::clamp(42., 1., 100., less<double>()) == 42.);
	EATEST_VERIFY(std::clamp(-42., 1., 100., less<double>()) == 1.);
	EATEST_VERIFY(std::clamp(420., 1., 100., less<double>()) == 100.);
	EATEST_VERIFY(std::clamp(1., 1., 100., less<double>()) == 1.);
	EATEST_VERIFY(std::clamp(100., 1., 100., less<double>()) == 100.);

	EATEST_VERIFY(std::clamp(A(42), A(1), A(100), LessStruct()).a == A(42).a);
	EATEST_VERIFY(std::clamp(A(-42), A(1), A(100), LessStruct()).a == A(1).a);
	EATEST_VERIFY(std::clamp(A(420), A(1), A(100), LessStruct()).a == A(100).a);
	EATEST_VERIFY(std::clamp(A(1), A(1), A(100), LessStruct()).a == A(1).a);
	EATEST_VERIFY(std::clamp(A(100), A(1), A(100), LessStruct()).a == A(100).a);
...
int TestAlgorithm()
{
	...
	// bool none_of(InputIterator first, InputIterator last, Predicate p);
	{
		...
		EATEST_VERIFY(std::all_of( v.begin(), v.end(), DivisibleBy(2)));
		EATEST_VERIFY(std::any_of( v.begin(), v.end(), DivisibleBy(3)));
		EATEST_VERIFY(std::none_of(v.begin(), v.end(), DivisibleBy(5)));
	}
...

	{
		...
		std::pair<int*, int*> pairInt = mismatch(intArray1, intArray1, intArray2);
...

	{
		// OutputIterator move(InputIterator first, InputIterator last, OutputIterator result)
		{
			...
			for(eastl_size_t i = 0; i < 4; i++)
				src.push_back(std::string(1, (char8_t)('0' + i)));
...

	{
		...
		std::vector<uint64_t> vU64(kMaxSize, 0);
		std::vector< int64_t> vI64(kMaxSize, 0);
		std::vector<uint32_t> vU32(kMaxSize, 0);
		std::vector< int32_t> vI32(kMaxSize, 0);
		std::vector<uint16_t> vU16(kMaxSize, 0);
		std::vector< int16_t> vI16(kMaxSize, 0);

		for(eastl_size_t i = 0; i < kMaxSize; ++i)
		{
			std::fill(vU64.begin(), vU64.begin() + i, UINT64_C(0x0123456789abcdef));
			EATEST_VERIFY(EA::StdC::Memcheck64(&vU64[0], UINT64_C(0x0123456789abcdef), i) == NULL);
			EA::StdC::Memset64(&vU64[0], 0, i);

			std::fill(vI64.begin(), vI64.begin() + i, UINT64_C(0x0123456789abcdef));
			EATEST_VERIFY(EA::StdC::Memcheck64(&vI64[0], UINT64_C(0x0123456789abcdef), i) == NULL);
			EA::StdC::Memset64(&vI64[0], 0, i);

			std::fill(vU32.begin(), vU32.begin() + i, UINT32_C(0x01234567));
			EATEST_VERIFY(EA::StdC::Memcheck32(&vU32[0], UINT32_C(0x01234567), i) == NULL);
			EA::StdC::Memset32(&vU32[0], 0, i);

			std::fill(vI32.begin(), vI32.begin() + i, UINT32_C(0x01234567));
			EATEST_VERIFY(EA::StdC::Memcheck32(&vI32[0], UINT32_C(0x01234567), i) == NULL);
			EA::StdC::Memset32(&vI32[0], 0, i);

			std::fill(vU16.begin(), vU16.begin() + i, UINT16_C(0x0123));
			EATEST_VERIFY(EA::StdC::Memcheck16(&vU16[0], UINT16_C(0x0123), i) == NULL);
			EA::StdC::Memset16(&vU16[0], 0, i);

			std::fill(vI16.begin(), vI16.begin() + i, UINT16_C(0x0123));
		...
		{   // Regression for user-reported compile failure.
			...
			std::vector<int32_t> intArrayEnum;

			std::fill<std::vector<int32_t>::iterator, int32_t>(intArrayEnum.begin(), intArrayEnum.end(), eTestValue);
...

	{
		...
		std::vector<uint64_t>::iterator itU64 = std::fill_n(vU64.begin(), kMaxSize, UINT64_C(0x0123456789abcdef));
		...
		std::vector<int64_t>::iterator itI64 = std::fill_n(vI64.begin(), kMaxSize, UINT64_C(0x0123456789abcdef));
		...
		std::vector<uint32_t>::iterator itU32 = std::fill_n(vU32.begin(), kMaxSize, UINT32_C(0x01234567));
		...
		std::vector<int32_t>::iterator itI32 = std::fill_n(vI32.begin(), kMaxSize, UINT32_C(0x01234567));
		...
		std::vector<uint16_t>::iterator itU16 = std::fill_n(vU16.begin(), kMaxSize, UINT16_C(0x0123));
		...
		std::vector<int16_t>::iterator itI16 = std::fill_n(vI16.begin(), kMaxSize, UINT16_C(0x0123));
	...
	// set_difference_2
	{
		...
		{
			const std::vector<int> v1 = {1, 2, 4, 5,   7, 7, 9};
			const std::vector<int> v2 = {   2,       6,      9};
			std::vector<int> only_v1, only_v2;

			std::set_difference_2(v1.begin(), v1.end(), v2.begin(), v2.end(),
									std::inserter(only_v1, only_v1.begin()),
									std::inserter(only_v2, only_v2.begin()));

			EATEST_VERIFY((only_v1 == std::vector<int>{1, 4, 5, 7, 7}));
			EATEST_VERIFY((only_v2 == std::vector<int>{6}));
		}
        ...
        //                       OutputIterator result1, OutputIterator result2, Compare compare)
        {
			...
			const std::vector<local> v1 = {{1}, {2}, {4}, {5},      {7}, {7}, {9}};
			const std::vector<local> v2 = {     {2},           {6},           {9}};
			std::vector<local> only_v1, only_v2;
			...
			EATEST_VERIFY((only_v1 == std::vector<local>{{1}, {4}, {5}, {7}, {7}}));
			EATEST_VERIFY((only_v2 == std::vector<local>{{6}}));
		}
	}
	...
	// set_decomposition
	{
		...
		//                           OutputIterator1 result1, OutputIterator2 result2, OutputIterator3 result3)
		{
			const std::vector<int> v1 = {1, 2, 4, 5,   7, 7, 9};
			const std::vector<int> v2 = {   2,       6,      9};
			std::vector<int> only_v1, only_v2, intersection;

			std::set_decomposition(v1.begin(), v1.end(), v2.begin(), v2.end(),
									std::inserter(only_v1, only_v1.begin()),
									std::inserter(only_v2, only_v2.begin()),
			...
			EATEST_VERIFY((only_v1 == std::vector<int>{1, 4, 5, 7, 7}));
			EATEST_VERIFY((only_v2 == std::vector<int>{6}));
			EATEST_VERIFY((intersection == std::vector<int>{2, 9}));
		}
		...
		//                           OutputIterator1 result1, OutputIterator2 result2, OutputIterator3 result3, Compare compare)
		{
			...
			const std::vector<local> v1 = {{1}, {2}, {4}, {5},      {7}, {7}, {9}};
			const std::vector<local> v2 = {     {2},           {6},           {9}};
			std::vector<local> only_v1, only_v2, intersection;
			...
			EATEST_VERIFY((only_v1 == std::vector<local>{{1}, {4}, {5}, {7}, {7}}));
			EATEST_VERIFY((only_v2 == std::vector<local>{{6}}));
			EATEST_VERIFY((intersection == std::vector<local>{{2}, {9}}));
		}
	}

    {
...

		{
			...
			// Test an empty set.
			EATEST_VERIFY(std::is_permutation(intArray1, intArray1 + 0, intArray2));

			// Test two identical sets.
			EATEST_VERIFY(std::is_permutation(intArray1, intArray1 + EAArrayCount(intArray1), intArray2));
			std::random_shuffle(intArray1, intArray1 + EAArrayCount(intArray1), eastlRNG);

			// Test order randomization.
			EATEST_VERIFY(std::is_permutation(intArray1, intArray1 + EAArrayCount(intArray1), intArray2));
			std::random_shuffle(intArray2, intArray2 + EAArrayCount(intArray2), eastlRNG);
			EATEST_VERIFY(std::is_permutation(intArray1, intArray1 + EAArrayCount(intArray1), intArray2));
		...
		for(int n = 0; n < 100000; n++)
		{
			...
			bool isPermutation = std::is_permutation(intArray1, intArray1 + EAArrayCount(intArray1), intArray2);

			// If is_permutation returned true, then sorted versions of the two arrays should be identical.
			std::sort(intArray1, intArray1 + EAArrayCount(intArray1));
			std::sort(intArray2, intArray2 + EAArrayCount(intArray2));

			std::pair<eastl_size_t*, eastl_size_t*> mismatchResult = std::mismatch(intArray1, intArray1 + EAArrayCount(intArray1), intArray2);
...

	{
		...
		do {
			++count;
		} while(next_permutation(intArray.begin(), intArray.end(), std::less<int>()));
		EATEST_VERIFY(count == 40320); // count = n!
		EATEST_VERIFY(is_sorted(intArray.begin(), intArray.end()));
	}
...

	{
		...
		// std::array (ContiguousIterator/Pointer)
		const eastl_size_t kRotateArraySize = 10;
		typedef std::array<int, kRotateArraySize> IntArray;
		...
		{   // This type is templated, so we can't run a loop over various sizes.
			...
			{
				std::generate_n(intArray.begin(), kRotateArraySize, GenerateIncrementalIntegers<int>());
				IntArray::iterator intArrayItMiddle = std::next(intArray.begin(), i);
				IntArray::iterator intArrayIt = std::rotate(intArray.begin(), intArrayItMiddle, intArray.end());
		...
		for(eastl_size_t s = 10; s < 500; s += (eastl_size_t)rng.RandRange(50, 100))
		{
			...
			{
				std::generate_n(intVector.begin(), s, GenerateIncrementalIntegers<int>());
				IntVector::iterator intVectorItMiddle = std::next(intVector.begin(), i);
				IntVector::iterator intVectorIt = std::rotate(intVector.begin(), intVectorItMiddle, intVector.end());
		...
		for(eastl_size_t s = 10; s < 500; s += (eastl_size_t)rng.RandRange(50, 100))
		{
			...
			for(eastl_size_t i = 0; i < s; i++)
			{
				std::generate_n(intDeque.begin(), s, GenerateIncrementalIntegers<int>());
				IntDeque::iterator intDequeItMiddle = std::next(intDeque.begin(), i);
				IntDeque::iterator intDequeIt = std::rotate(intDeque.begin(), intDequeItMiddle, intDeque.end());
		...
		for(eastl_size_t s = 10; s < 500; s += (eastl_size_t)rng.RandRange(50, 100))
		{
			...
			for(eastl_size_t i = 0; i < s; i++)
			{
				std::generate_n(intList.begin(), s, GenerateIncrementalIntegers<int>());
				IntList::iterator intListItMiddle = std::next(intList.begin(), i);
				IntList::iterator intListIt = std::rotate(intList.begin(), intListItMiddle, intList.end());
		...
		// std::slist (ForwardIterator)
		typedef std::slist<int> IntSlist;
		...
		for(eastl_size_t s = 10; s < 500; s += (eastl_size_t)rng.RandRange(50, 100))
		{
			...
			for(eastl_size_t i = 0; i < s; i++)
			{
				std::generate_n(intSlist.begin(), s, GenerateIncrementalIntegers<int>());
				IntSlist::iterator intSlistItMiddle = std::next(intSlist.begin(), i);
				IntSlist::iterator intSlistIt = std::rotate(intSlist.begin(), intSlistItMiddle, intSlist.end());
	...
	// test std::sort with move-only type
	{
		{
			std::vector<std::unique_ptr<int>> vec;
			std::sort(vec.begin(), vec.end(), [](const std::unique_ptr<int>& lhs, const std::unique_ptr<int>& rhs) { return *lhs < *rhs; });
		}
		{
			std::vector<std::unique_ptr<int>> vec;
		...
		{
			std::vector<MissingMoveConstructor> vec;
			std::sort(vec.begin(), vec.end(), [](const MissingMoveConstructor& lhs, const MissingMoveConstructor& rhs) { return lhs < rhs; });
		}
		{
			std::vector<MissingMoveConstructor> vec;
			std::sort(vec.begin(), vec.end());
		...
		{
			std::vector<MissingMoveAssignable> vec;
			std::sort(vec.begin(), vec.end(), [](const MissingMoveAssignable& lhs, const MissingMoveAssignable& rhs) { return lhs < rhs; });
		}
		{
			std::vector<MissingMoveAssignable> vec;
			std::sort(vec.begin(), vec.end());
		...
		}
		{
			for (unsigned tests = 0; tests < 50; ++tests)
			{
				...
				auto vec1Cmp = [](const std::unique_ptr<int>& lhs, const std::unique_ptr<int>& rhs) { return *lhs < *rhs; };
				std::sort(vec1.begin(), vec1.end(), vec1Cmp);
				EATEST_VERIFY(std::is_sorted(vec1.begin(), vec1.end(), vec1Cmp));
			}
		}
	}
