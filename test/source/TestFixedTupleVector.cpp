...
#include <EASTL/bonus/fixed_tuple_vector>
...
int TestFixedTupleVectorVariant()
{
	...
	// Test uninit'ed push-backs
	{
		...
		// verify the equivalent accessors for the const container exist/compile
		{
			...
			static_assert(std::is_same<decltype(constVec.begin()), decltype(constVec.cbegin())>::value, "error");
			static_assert(std::is_same<decltype(constVec.end()), decltype(constVec.cend())>::value, "error");
			static_assert(std::is_same<decltype(constVec.rbegin()), decltype(constVec.crbegin())>::value, "error");
			static_assert(std::is_same<decltype(constVec.rend()), decltype(constVec.crend())>::value, "error");

			// check that return type of non-const version of begin and cbegin (etc) do _not_ match
			static_assert(!std::is_same<decltype(complexVec.begin()), decltype(complexVec.cbegin())>::value, "error");
			static_assert(!std::is_same<decltype(complexVec.end()), decltype(complexVec.cend())>::value, "error");
			static_assert(!std::is_same<decltype(complexVec.rbegin()), decltype(complexVec.crbegin())>::value, "error");
			static_assert(!std::is_same<decltype(complexVec.rend()), decltype(complexVec.crend())>::value, "error");
		}
	}
	...
	// Test tuple_Vector in a ranged for, and other large-scale iterator testing
	{
		...
		// test simple iteration, and reverse iteration
		{
			...
			std::for_each(tripleElementVec.rbegin(), tripleElementVec.rend(),
