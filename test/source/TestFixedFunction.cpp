...
int TestFixedFunctionDtor()
{
...

	{
		...
		TestObject::Reset();
		{
			std::fixed_function<sizeof(TestObject), void(void)> ff = [to] {};
			ff();
		}
...
int TestFixedFunctionStdBind()
{
...

	{
		...
		TestObject::Reset();
		{
			std::fixed_function<64, void(void)> ff = std::bind(lambda);
			ff();
		}
		VERIFY(TestObject::IsClear());
		VERIFY(val == 1);
	}
	{
		...
		TestObject::Reset();
		{
			std::fixed_function<64, void(void)> ff = nullptr;
		    ff = std::bind(lambda);
			ff();
		}
		VERIFY(TestObject::IsClear());
...
int TestFixedFunctionReferenceWrapper()
{
...

	{
		...
		TestObject::Reset();
		{
			std::fixed_function<sizeof(std::reference_wrapper<decltype(lambda)>), void(void)> ff = std::reference_wrapper<decltype(lambda)>(lambda);
			ff();
		}
		VERIFY(TestObject::IsClear());
		VERIFY(val == 1);
	}
	{
		...
		TestObject::Reset();
		{
			std::fixed_function<sizeof(std::reference_wrapper<decltype(lambda)>), void(void)> ff = nullptr;
		    ff = std::reference_wrapper<decltype(lambda)>(lambda);
			ff();
		}
		VERIFY(TestObject::IsClear());
		VERIFY(val == 2);
...
int TestFixedFunctionFunctionPointer()
{
	...
	{
		std::fixed_function<sizeof(FuncPtrVoid), void(int*)> ff = &TestVoidRet;
		ff(&val);
		VERIFY(val == 1);
	}
	{
		std::fixed_function<sizeof(FuncPtrVoid), void(int*)> ff;
		ff = &TestVoidRet;
		ff(&val);
		VERIFY(val == 2);
	}
	{
		std::fixed_function<sizeof(FuncPtrInt), int(int*)> ff = &TestIntRet;
		int ret = ff(&val);
		VERIFY(ret == 2);
		VERIFY(val == 3);
	}
	{
		std::fixed_function<sizeof(FuncPtrInt), int(int*)> ff;
		ff = &TestIntRet;
		int ret = ff(&val);
		VERIFY(ret == 3);
		VERIFY(val == 4);
...
int TestFixedFunctionPointerToMemberFunction()
{
	...
	{
		std::fixed_function<sizeof(PTMFSize), void(const TestVoidRet&)> ff = static_cast<void(TestVoidRet::*)() const>(&TestVoidRet::IncX);
		ff(cvoidRet);
		VERIFY(cvoidRet.x == 1);
	}
	{
	...
	{
		std::fixed_function<sizeof(PTMFSize), void(TestVoidRet&)> ff = static_cast<void(TestVoidRet::*)()>(&TestVoidRet::IncX);
		ff(voidRet);
		VERIFY(voidRet.x == 2);
	}

	{
		std::fixed_function<sizeof(PTMFSize), int(const TestIntRet&)> ff = static_cast<int(TestIntRet::*)() const>(&TestIntRet::IncX);
		int ret = ff(cintRet);
		VERIFY(ret == 0);
		VERIFY(cintRet.x == 1);
	}
	...
	{
		std::fixed_function<sizeof(PTMFSize), int(TestIntRet&)> ff = static_cast<int(TestIntRet::*)()>(&TestIntRet::IncX);
		int ret = ff(intRet);
		VERIFY(ret == 1);
		VERIFY(intRet.x == 2);
	}
...
int TestFixedFunctionPointerToMemberData()
{
	...
	{
		std::fixed_function<sizeof(void*), int(const Test&)> ff = &Test::x;
		int ret = ff(t);
		VERIFY(ret == 1);
	}
	{
	...
	{
		std::fixed_function<sizeof(void*), int(const Test&)> ff;
		ff = &Test::x;
		int ret = ff(t);
		VERIFY(ret == 1);
	}
...
int TestFixedFunctionExistingClosure()
{
...

	{
		TestObject to;
		{
			using ff_t = std::fixed_function<sizeof(TestObject), void(void)>;
			{
				ff_t ff1 = [to] {};
				ff_t  ff3 = [to] {};
				TestObject::Reset();
			...
			}
			{
				...
				{
					ff_t ff2 = ff1;
					ff2 = std::move(ff3);  // copy over function that holds existing closure state
				}
				VERIFY(TestObject::IsClear());
			}
			{
...
int TestFixedFunctionBasic()
{
	...
	{
		std::fixed_function<0, void(void)> fn;
	...
	{
		std::fixed_function<0, int(int)> fn = [](int param) { return param; };
		EATEST_VERIFY(fn(42) == 42);
	}

	{
		std::fixed_function<0, int(int)> fn = ReturnVal;
		EATEST_VERIFY(fn(42) == 42);
	}

	{
		std::fixed_function<0, int()> fn0 = ReturnZero;
		std::fixed_function<0, int()> fn1 = ReturnOne;
	...
	{
		std::fixed_function<0, int(int)> f1(nullptr);
		EATEST_VERIFY(!f1);

		std::fixed_function<0, int(int)> f2 = nullptr;
		EATEST_VERIFY(!f2);
	}

	{
		...
		{
			std::fixed_function<48, uint64_t(void)> fn = large_add;
			auto result = fn();
			EATEST_VERIFY(result == 21);
		}

		{
			std::fixed_function<sizeof(large_add), uint64_t(void)> fn = large_add;
			auto result = fn();
			EATEST_VERIFY(result == 21);
		}
	}
	...
	{
		using ff_0 = std::fixed_function<0, int(int)>;
		using ff_1 = std::fixed_function<1, int(int)>;
		using ff_4 = std::fixed_function<4, int(int)>;
		using ff_8 = std::fixed_function<8, int(int)>;
		using ff_64 = std::fixed_function<64, int(int)>;
		using ff_128 = std::fixed_function<128, int(int)>;
		using ff_4096 = std::fixed_function<4096, int(int)>;
	...
	// Verify conversions to fixed_function<N> for sizes greater or equal to the source size.
	{
		...
		std::fixed_function<8, uint32_t(void)> ff8 = [v0, v1]
			{ return v0 + v1; };

		{
			std::fixed_function<16, uint32_t(void)> ff16(ff8);
			VERIFY(result == ff16());
		}
		...
		{
			std::fixed_function<16, uint32_t(void)> ff16; 
			ff16 = ff8;
			VERIFY(result == ff16());
		}
		...
		{
			auto ff8Copy = ff8;
			std::fixed_function<16, uint32_t(void)> ff16(std::move(ff8Copy));
			VERIFY(result == ff16());
		}
