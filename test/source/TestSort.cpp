...
namespace std
{
	namespace Internal
	{
		...
		//
		// Example usage:
		//     std::sort(floatArray.begin(), floatArray.end(), SafeFloatCompare());
		//
...
int TestSort()
{
...

	{
		...
		EATEST_VERIFY( is_sorted(array + 11, array + 23, std::greater<int>()));
	}

	{
		...
		EATEST_VERIFY( is_sorted_until(sorted + EAArrayCount(sorted),  sorted + EAArrayCount(sorted), std::less<int>()) == sorted + EAArrayCount(sorted) );
		EATEST_VERIFY( is_sorted_until(notsorted +  2, notsorted +  8, std::less<int>()) == notsorted + 6 );
	}
	...
	// Sort arrays of size 0 - N. Sort M random permutations of each.
	{
		...
		for(int i = 0; i < (150 + (gEASTL_TestLevel * 200)); i += (i < 5) ? 1 : 37) // array sizes of 0 to 300 - 2100, depending on test level.
		{
			...
			const int64_t expectedSum = std::accumulate(begin(intArraySaved), end(intArraySaved), int64_t(0));

			for(int j = 0; j < 300 + (gEASTL_TestLevel * 50); j++)
			{
				std::random_shuffle(intArraySaved.begin(), intArraySaved.end(), rng);

				intArray = intArraySaved;
				bubble_sort(intArray.begin(), intArray.end());
				EATEST_VERIFY(is_sorted(intArray.begin(), intArray.end()));
				EATEST_VERIFY(std::accumulate(begin(intArraySaved), end(intArraySaved), int64_t(0)) == expectedSum);
	...
	// Test insertion_sort() does not invalidate a BidirectionalIterator by doing --BidirectionalIterator.begin()
	{
		...
		insertion_sort(deque.begin(), deque.end(), std::less<int>{});
	}
	...
	TestObject::Reset();
	{
		...
		for(int i = 0; i < (150 + (gEASTL_TestLevel * 200)); i += (i < 5) ? 1 : 37) // array sizes of 0 to 300 - 2100, depending on test level.
		{
			...
			for(int j = 0; j < 300 + (gEASTL_TestLevel * 50); j++)
			{
				std::random_shuffle(toArraySaved.begin(), toArraySaved.end(), rng);
...

	{
		...
		for(i = 1; i < 16; i++)
		{
			for(j = 0; j < i; j++)
			{
				std::copy(intArrayInit, intArrayInit + i, intArraySorted);
				std::sort(intArraySorted, intArraySorted + i);

				std::copy(intArrayInit, intArrayInit + i, intArray);
		...
		for(i = 1; i < 16; i++)
		{
			for(j = 0; j < i; j++)
			{
				...
				std::copy(intArrayInit, intArrayInit + 16, intArray);
				nth_element(intArray, intArray + j, intArray + i, std::less<int>());
				EATEST_VERIFY(intArray[j] == intArraySorted[j]);
			}
		}
	}
	...
	{ 
		...
		typedef std::less<int> IntCompare;
...

	{
		...
		std::random_shuffle(intDequeSaved.begin(), intDequeSaved.end(), rng);
...

	{
		auto test_stable_sort = [&](auto testArray, size_t count)
		{
			...
			std::copy_if(testArray.begin(), testArray.end(), std::back_inserter(evenArray), isEven);
			std::copy_if(testArray.begin(), testArray.end(), std::back_inserter(oddArray), isOdd);

			const auto boundary = std::stable_partition(testArray.begin(), testArray.end(), isEven);

			const auto evenCount = std::distance(testArray.begin(), boundary);
			const auto oddCount = std::distance(boundary, testArray.end());
			...
			EATEST_VERIFY(std::equal(testArray.begin(), boundary, evenArray.begin()));
			EATEST_VERIFY(std::equal(boundary, testArray.end(), oddArray.begin()));
		};
