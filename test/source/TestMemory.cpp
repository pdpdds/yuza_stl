...
typedef std::vector<AssetHandler> AssetHandlerArray;
...
int TestValueInitOptimization()
{
	...
	{
		std::vector<T> v1(ELEM_COUNT);
		std::vector<ValueInitOf<T>> v2(ELEM_COUNT);
...
std::late_constructed<LCTestObject, true, true>  gLCTestObjectTrueTrue;
std::late_constructed<LCTestObject, false, true> gLCTestObjectFalseTrue;
std::late_constructed<LCTestObject, false, false> gLCTestObjectFalseFalse;
std::late_constructed<LCTestObject, true, false> gLCTestObjectTrueFalse;
...
int TestMemory()
{
...

	{
		...
		static_assert(std::alignment_of<typename late_constructed<LCTestObject>::value_type>::value == 64, "late_constructed alignment failure.");
		static_assert(std::alignment_of<typename late_constructed<LCTestObject>::storage_type>::value == 64, "late_constructed alignment failure.");
		static_assert(std::alignment_of<late_constructed<LCTestObject> >::value >= 64, "late_constructed alignment failure.");
	...
	LCTestObject::sTODtorCount = 0;
	{
		std::late_constructed<LCTestObject, true, false> lc;
		lc.construct();
	}
	EATEST_VERIFY((LCTestObject::sTOCount == 1) && (LCTestObject::sTOCtorCount == 1) && (LCTestObject::sTODtorCount == 0));
	...
	LCTestObject::sTODtorCount = 0;
	{
		std::late_constructed<LCTestObject, false, false> lc;
		lc.construct();
	}
	EATEST_VERIFY((LCTestObject::sTOCount == 1) && (LCTestObject::sTOCtorCount == 1) && (LCTestObject::sTODtorCount == 0));
	...
	int* pEnd = std::uninitialized_relocate_start<int*, int*>((int*)NULL, (int*)NULL, (int*)NULL);
	EATEST_VERIFY(pEnd == NULL);

	pEnd = std::uninitialized_relocate_commit<int*, int*>((int*)NULL, (int*)NULL, (int*)NULL);
	EATEST_VERIFY(pEnd == NULL);

	pEnd = std::uninitialized_relocate_abort<int*, int*>((int*)NULL, (int*)NULL, (int*)NULL);
	EATEST_VERIFY(pEnd == NULL);

	pEnd = std::uninitialized_relocate<int*, int*>((int*)NULL, (int*)NULL, (int*)NULL);
	EATEST_VERIFY(pEnd == NULL);
	...
	pEnd = std::uninitialized_copy<int*, int*>((int*)NULL, (int*)NULL, (int*)NULL);
	EATEST_VERIFY(pEnd == NULL);
	...
	pEnd = std::uninitialized_copy_ptr<int*, int*, int*>((int*)NULL, (int*)NULL, (int*)NULL);
	EATEST_VERIFY(pEnd == NULL);
	...
	std::uninitialized_fill<int*, int>((int*)NULL, (int*)NULL, (int)0);
	...
	std::uninitialized_fill_ptr<int>((int*)NULL, (int*)NULL, (int)0);
	...
	std::uninitialized_fill_n<int*, int, int>((int*)NULL, (int)0, (int)0);
	...
	std::uninitialized_fill_n_ptr<int, int>((int*)NULL, (int)0, (int)0);
	...
	std::uninitialized_copy_fill<int*, int*, int>((int*)NULL, (int*)NULL, (int*)NULL, (int*)NULL, (int)0);
	...
	std::uninitialized_fill_copy<int*, int, int*>((int*)NULL, (int*)NULL, (int)0, (int*)NULL, (int*)NULL);
	...
	std::uninitialized_copy_copy<int*, int*, int*>((int*)NULL, (int*)NULL, (int*)NULL, (int*)NULL, (int*)NULL);
	...
	// uninitialized_default_construct
	{
		...
		std::uninitialized_default_construct(pTestMemory, pTestMemory + 10);
	...
	// uninitialized_default_construct_n
	{
		...
		auto endIter = std::uninitialized_default_construct_n(pTestMemory, 5);
	...
	// uninitialized_value_construct
	{
		...
		std::uninitialized_value_construct(pTestMemory, pTestMemory + 10);
		EATEST_VERIFY(TestObject::sTODefaultCtorCount == 10);
	}
	...
	// uninitialized_value_construct_n
	{
		...
		auto endIter = std::uninitialized_value_construct_n(pTestMemory, 5);
		EATEST_VERIFY(TestObject::sTODefaultCtorCount == 5);
		EATEST_VERIFY(endIter == (pTestMemory + 5));
	}
	...
	{
		// Regression for user reported operator new problem (12/8/2009):
		std::vector<AssetHandler> ahArray;
	...
	// void* align_advance(size_t alignment, size_t size, void*& ptr, size_t& space);
	{
		...
		// It's a coincidence that we are using std::align to set up a buffer for testing std::align below.
		ptrSaved = std::align(256, 256, ptr, space);
		...
		for(size_t a = 1; a < 64; a *= 2)
		{
			...
			{
				ptrAligned = std::align(a, a, ptr, space);
		...
		for(size_t a = 1; a < 64; a *= 2)
		{
			...
			for(i = 0, space = 256, ptr = ptrSaved; i < kBufferSize; i += a)
			{
				ptrAligned = std::align_advance(a, a, ptr, space, &ptr, &space);
...

	{
		...
		ptr      = (char*)ptr - space;
		ptrSaved = ptr;
		pResult  = std::align(1, space + 1, ptr, space);             // Possible alignment, impossible size due to wraparound.
		...
		ptr      = (char*)ptr - space;
		ptrSaved = ptr;
		pResult  = std::align(space * 2, 32, ptr, space);            // Impossible alignment due to wraparound, possible size.
		EATEST_VERIFY((pResult == NULL) && (ptr == ptrSaved));
	}
