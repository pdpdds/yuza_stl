...
int TEST_STRING_NAME()
{
	int nErrorCount = 0;
	{
		...
		// EA_CONSTEXPR basic_string_view(const basic_string_view& other) = default;
		{
			...
			VERIFY(std::Compare(sw1.data(), sw2.data(), sw1.size()) == 0);
		}
		...
		// EA_CONSTEXPR basic_string_view(const T* s, size_type count)
		{
...

			{
				...
				VERIFY(sw.size() == 5);
				VERIFY(sw.size() == sw.length());
				VERIFY(std::Compare(sw.data(), LITERAL("Hello"), sw.size()) == 0);
			}
		}
		...
		// EA_CONSTEXPR basic_string_view(const T* s)
		{
			...
			VERIFY(sw.size() == 17);
			VERIFY(sw.size() == sw.length());
			VERIFY(std::Compare(sw.data(), pLiteral, sw.size()) == 0);
		}
		...
		// basic_string_view& operator=(const basic_string_view& view) = default;
		{
			...
			VERIFY(!sw2.empty());
			VERIFY(sw1.size() == sw2.size());
			VERIFY(std::Compare(sw1.data(), pLiteral, sw1.size()) == 0);
			VERIFY(std::Compare(sw2.data(), pLiteral, sw2.size()) == 0);
		}
		...
		// EA_CONSTEXPR const_pointer data() const
		{
			...
			VERIFY(sw.data() != nullptr);
			VERIFY(std::Compare(sw.data(), pLiteral, sw.size()) == 0);
			VERIFY(std::Compare(sw.data() + 11, LITERAL("Canada"), 6) == 0);
		}
		...
		// EA_CONSTEXPR bool empty() const EA_NOEXCEPT 
		{
			...
			VERIFY(sw.size() == 51); 
			VERIFY(sw.length() == 51); 
			VERIFY(sw.max_size() == std::numeric_limits<typename StringViewT::size_type>::max()); 
			VERIFY(!sw.empty()); 
		}
		...
		// EA_CONSTEXPR void swap(basic_string_view& v)
		{
			...
			StringViewT sw2(pC);
			sw1.swap(sw2);
			VERIFY(std::Compare(sw1.data(), pC, sw1.size()) == 0);
			VERIFY(std::Compare(sw2.data(), pV, sw2.size()) == 0);
		}
		...
		// EA_CONSTEXPR void remove_suffix(size_type n)
		{
			StringViewT sw(LITERAL("Vancouver"));
			sw.remove_prefix(3);
			VERIFY(std::Compare(sw.data(), LITERAL("couver"), sw.size()) == 0);
			VERIFY(sw.size() == 6);

			sw.remove_prefix(3);
			VERIFY(std::Compare(sw.data(), LITERAL("ver"), sw.size()) == 0);
			VERIFY(sw.size() == 3);

			sw.remove_suffix(1);
			VERIFY(std::Compare(sw.data(), LITERAL("ve"), sw.size()) == 0);
			VERIFY(sw.size() == 2);
			
			sw.remove_suffix(1);
			VERIFY(std::Compare(sw.data(), LITERAL("v"), sw.size()) == 0);
			VERIFY(sw.size() == 1);

			sw.remove_suffix(1);
			VERIFY(std::Compare(sw.data(), LITERAL(""), sw.size()) == 0);
			VERIFY(sw.size() == 0);
		}
		...
		// size_type copy(T* s, size_type n, size_type pos = 0) const;
		{
			...
			VERIFY(std::Compare(buf, LITERAL("Hello"), 5) == 0);
			VERIFY(cnt == 5);
		}
		...
		// EA_CONSTEXPR basic_string_view substr(size_type pos = 0, size_type n = npos) const;
		{
			...
			VERIFY(std::Compare(sw2.data(), LITERAL("Hello"), sw2.size()) == 0);
		}
		...
		// template<> struct hash<std::u32string_view>;
		{
			...
			VERIFY(std::hash<StringViewT>{}(sw1) != std::hash<StringViewT>{}(sw2));
			VERIFY(std::hash<StringViewT>{}(sw2) == std::hash<StringViewT>{}(sw3));
			VERIFY(std::hash<StringViewT>{}(sw3) == std::hash<decltype(s)>{}(s));
		}
	}
