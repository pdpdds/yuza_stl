...
bool VerifyListMapSequence(const char* pName, 
							...
							T1 t15 = 0, T2 t25 = 0)
{
	typename std::list_map<T1, T2>::iterator it = listMap.begin();
...
int TestListMap()
{
	...
	{
		typedef std::list_map<uint32_t, uint64_t> TestMapType;
		typedef std::pair<uint32_t, uint64_t>     ValueType;      // We currently can't use TestMapType::value_type because its 'first' is const.
		...
		EATEST_VERIFY(testMap.validate_iterator(testMap.find(8)) == (std::isf_valid | std::isf_current | std::isf_can_dereference));
		EATEST_VERIFY(testMap.validate_iterator(testMap.find(30)) == (std::isf_valid | std::isf_current));
		EATEST_VERIFY(testMap.validate_iterator(tempIter) == std::isf_none);
		EATEST_VERIFY(testMap.validate());
	...
	{
		typedef std::list_map<std::string, uint32_t> TestStringMapType;
		TestStringMapType           testStringMap;
		TestStringMapType::iterator strIter;

		testStringMap.push_back(std::string("hello"), 750);
		EATEST_VERIFY(testStringMap.size() == 1);

		strIter = testStringMap.find_as("hello", std::less_2<std::string, const char*>());
		EATEST_VERIFY(strIter != testStringMap.end());
		EATEST_VERIFY(strIter->first == "hello");
		EATEST_VERIFY(strIter->second == 750);

		strIter = testStringMap.find_as("fake_string", std::less_2<std::string, const char*>());
		EATEST_VERIFY(strIter == testStringMap.end());
