...
int TestTuple()
{
...

	{
		...
		const bool&& constRval = get<bool>(std::move(aTupleWithRepeatedType));
	...
	// apply
	{
		...
		{
			{
				auto result = std::apply([](int i) { return i; }, make_tuple(1));
				EATEST_VERIFY(result == 1);
			}

			{
				auto result = std::apply([](int i, int j) { return i + j; }, make_tuple(1, 2));
				EATEST_VERIFY(result == 3);
			}


			{
				auto result = std::apply([](int i, int j, int k, int m) { return i + j + k + m; }, make_tuple(1, 2, 3, 4));
		...
		// test with pair
		{
			auto result = std::apply([](int i, int j) { return i + j; }, make_pair(1, 2));
			EATEST_VERIFY(result == 3);
		}
		...
		{
			// TODO(rparolin):   
			// std::array requires std::get support before we can support unpacking std::arrays with std::apply.
			//
			// {
			//     auto result = std::apply([](int i) { return i; }, std::array<int, 1>{1});
			//     EATEST_VERIFY(result == 1);
			// }
			// {
			//     auto result = std::apply([](int i, int j) { return i + j; }, std::array<int, 2>{1,2});
			//     EATEST_VERIFY(result == 3);
			// }
			// {
			//     auto result = std::apply([](int i, int j, int k, int m) { return i + j + k + m; }, std::array<int, 4>{1, 2, 3, 4});
			//     EATEST_VERIFY(result == 10);
			// }
		}
	}
	...
	{
		std::tuple<int, float, TestObject> arrayTup[] = {
			{1, 1.0f, TestObject(1)},
			{2, 2.0f, TestObject(2)},
			{3, 3.0f, TestObject(3)},
			{4, 4.0f, TestObject(4)}
		...
		std::tuple<std::vector<float>, float> arrayOfArrayTup[] = {
			{1.0f, 1.0f},
			{2.0f, 2.0f}
		};

		std::tuple<std::vector<int>, float> arrayOfArrayTup2[] = {
			{1, 1.0f},
			{2, 2.0f}
		};
#endif
	...
	{
		void* empty = nullptr;
		auto t = std::make_tuple(empty, true);
		auto tc = std::tuple_cat(std::make_tuple("asd", 1), t);

		static_assert(std::is_same_v<decltype(tc), std::tuple<const char*, int, void*, bool>>, "type mismatch");

		EATEST_VERIFY(std::string("asd") == std::get<0>(tc));
		EATEST_VERIFY(std::get<1>(tc) == 1);
		EATEST_VERIFY(std::get<2>(tc) == nullptr);
		EATEST_VERIFY(std::get<3>(tc) == true);
	}
	...
	// user reported regression that exercises type_traits trying to pull out the element_type from "fancy pointers"
	{
		...
		auto t = std::make_tuple(std::move(up));

		using ResultTuple_t = decltype(t);
		static_assert(std::is_same_v<ResultTuple_t, std::tuple<std::unique_ptr<int[]>>>); 
		static_assert(std::is_same_v<std::tuple_element_t<0, ResultTuple_t>, std::unique_ptr<int[]>>);
	}
