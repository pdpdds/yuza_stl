...
namespace std
{
	...
	typedef basic_string<char, allocator> local_string8;  // collides with std::string8 in bulkbuilds
...
public:
	...
	fixed_pool_reference(std::fixed_pool& fixedPool)
	{
...
template class std::queue<int, deque<int> >;
template class std::queue<Align64, deque<Align64> >;
template class std::queue<TestObject, list<TestObject> >;
//template class std::queue<IntNode, intrusive_list<IntNode> >;// This test has been disabled as of the addition of initializer_list support to std::queue. initializer_lists have const nodes, which is incompatible with intrusive_list. You can use std::queue<IntNode, intrusive_list<IntNode> > as long as you don't use initializer_list with it. The problem with this line of code is that it forces compilation of the entire class.
...
static int TestQueue()
{
...

	{
		...
		queue<TestObject, vector<TestObject> > toQ_A(std::move(toQ_0), toQ_0.get_container().get_allocator()); // It would be better if we also tested an alternative allocator.
		EATEST_VERIFY(toQ_A.size() == 0);
		toQ_A.push(TestObject(1000));
		EATEST_VERIFY(toQ_A.size() == 1);

		queue<TestObject, vector<TestObject> > toQ_B(std::move(toQ_A), toQ_A.get_container().get_allocator()); // It would be better if we also tested an alternative allocator.
		EATEST_VERIFY((toQ_B.size() == 1) && toQ_A.empty());

		std::vector<TestObject> toVectorM(toVector);
		queue<TestObject, vector<TestObject> > toQ_C(std::move(toVectorM));
	...
	{ // Test std namespace elements contained in queue
		#ifndef EA_COMPILER_NO_STANDARD_CPP_LIBRARY
			std::queue< std::pair<int, int> > stlQueue;
...
// Template instantations.
// These tell the compiler to compile all the functions for the given class.
template class std::priority_queue<int, vector<int> >;
template class std::priority_queue<Align64, deque<Align64> >;
template class std::priority_queue<TestObject, vector<TestObject> >;
template class std::priority_queue<float, vector<float>, less<float> >;
...
static int TestPriorityQueue()
{
...

	{
		...
		priority_queue<TestObject, vector<TestObject> > toPQ_A(toPQ_0.get_container().begin(), toPQ_0.get_container().begin(), std::less<TestObject>(), toPQ_0.get_container());
		EATEST_VERIFY(toPQ_A.size() == 0);
		toPQ_A.push(TestObject(1000));
		EATEST_VERIFY(toPQ_A.size() == 1);

		priority_queue<TestObject, vector<TestObject> > toPQ_B(std::move(toPQ_A), toPQ_A.get_container().get_allocator()); // It would be better if we also tested an alternative allocator.
		EATEST_VERIFY((toPQ_B.size() == 1) && toPQ_A.empty());

		std::vector<TestObject> toVectorM(toVector);
		priority_queue<TestObject, vector<TestObject> > toPQ_C(std::less<TestObject>(), std::move(toVectorM));
...
// Template instantations.
// These tell the compiler to compile all the functions for the given class.
template class std::stack<int, vector<int> >;
template class std::stack<Align64, list<Align64> >;
template class std::stack<TestObject, vector<TestObject> >;
//template class std::stack<IntNode, intrusive_list<IntNode> >; // This test has been disabled as of the addition of initializer_list support to std::stack. initializer_lists have const nodes, which is incompatible with intrusive_list. You can use std::stack<IntNode, intrusive_list<IntNode> > as long as you don't use initializer_list with it. The problem with this line of code is that it forces compilation of the entire class.
...
static int TestStack()
{
...

	{
		...
		stack<TestObject, vector<TestObject> > toS_A(std::move(toS_0), toS_0.get_container().get_allocator()); // It would be better if we also tested an alternative allocator.
		EATEST_VERIFY(toS_A.size() == 0);
		toS_A.push(TestObject(1000));
		EATEST_VERIFY(toS_A.size() == 1);

		stack<TestObject, vector<TestObject> > toS_B(std::move(toS_A), toS_A.get_container().get_allocator()); // It would be better if we also tested an alternative allocator.
		EATEST_VERIFY((toS_B.size() == 1) && toS_A.empty());

		std::vector<TestObject> toVectorM(toVector);
		stack<TestObject, vector<TestObject> > toS_C(std::move(toVectorM));
...
{
   typedef typename std::call_traits<T>::param_type       param_type;
   typedef typename std::call_traits<T>::reference        reference;
   typedef typename std::call_traits<T>::const_reference  const_reference;
   typedef typename std::call_traits<T>::value_type       result_type;
...
static std::string AccumulateString(std::string s, int x)
{
...
static int TestNumeric()
{
	...
	std::generate(v.begin(), v.end(), GenerateIncrementalIntegers<int>(1));

	int sum = std::accumulate(v.begin(), v.end(), 100);
	...
	std::generate(v.begin(), v.end(), GenerateIncrementalIntegers<int>(1));
	int product = std::accumulate(v.begin(), v.end(), 100, AccumulateMultiply);
	EATEST_VERIFY(product == (100 * 1 * 2 * 3 * 4 * 5));

	std::generate(v.begin(), v.end(), GenerateIncrementalIntegers<int>(1));
	std::string s = std::accumulate(v.begin(), v.end(), std::string("0"), AccumulateString);
...
static int TestAdaptors()
{
	...
	// reverse lvalue container
	{
		...
		std::vector<int> original(begin(int_data), end(int_data));

		std::vector<int> reversed;
		for(auto& e : std::reverse(original))
			reversed.push_back(e);

		std::reverse(begin(original), end(original));
	...
	{
		int int_data[] = {1, 2, 3, 4, 5, 6, 7, 8, 9};
		const std::vector<int> original(begin(int_data), end(int_data));

		std::vector<int> reversed;
		for(auto& e : std::reverse(original))
			reversed.push_back(e);

		std::vector<int> reversed_original(original);
		std::reverse(begin(reversed_original), end(reversed_original));
	...
	// reverse rvalue container
	{
		...
		std::vector<int> reversed;
		for (auto& e : std::reverse(std::vector<int>(original)))
			reversed.push_back(e);
