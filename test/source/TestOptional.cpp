...
public:
	...
	{
		return m_optional.value_or(std::forward<U>(def));
	}
};
...
int TestOptional()
{
		...
		{
			{
				...
				auto o = std::make_optional<local>(42);
				VERIFY(o.value().payload1 == 42);
			}
			{
				struct local { int payload1; int payload2; };
				auto o = std::make_optional<local>(42, 43);
				VERIFY(o.value().payload1 == 42);
    ...
    {
        move_test t;
        optional<move_test> o(std::move(t));
...

	{
...

		{
			...
			o1 = optional<assignment_test>(assignment_test());
			VERIFY(assignment_test::num_objects_inited == 1);
			optional<assignment_test> o2(std::move(o1));
			VERIFY(assignment_test::num_objects_inited == 2);
			o1 = nullopt;
			VERIFY(assignment_test::num_objects_inited == 1);
		}
	...
	{
		{
			// verify that the hash an empty std::optional object is zero.
...

	{
		...
		static_assert(!std::is_trivially_destructible_v<local_struct>, "");

		{
			local_struct ls;
			std::optional<local_struct> o{ls};
		}
		{
	...
	{
		{
			...
			VERIFY( o.value().data() == std::string("Hello World"));
			VERIFY(co.value().data() == std::string("Hello World"));
		}
		{
			// user regression
			...
			std::optional<local> o1 = local{std::make_unique<int>(42)};
			std::optional<local> o2;
		...
		}
		{
			...
			std::optional<local> n;
			std::optional<local> o1(n);
			VERIFY(!copyCtorCalledWithUninitializedValue);
			std::optional<local> o2(std::move(n));
