...
int TestLruCache()
{
	...
	// Test simple situation
	{
		...
		std::lru_cache<int, Foo> lruCache(3);
	...
	// Test more advanced creation / deletion via callbacks
	{
		...
		std::lru_cache<int, Foo*> lruCache(3, EASTLAllocatorType("eastl lru_cache"), createCallback, deleteCallback);
	...
	{
		std::lru_cache<int, int> lc = {{0, 10}, {1, 11}, {2, 12}, {3, 13}, {4, 14}, {5, 15}};
