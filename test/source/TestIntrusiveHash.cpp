...
#include <EASTL/intrusive_hash_map>
...
template class std::intrusive_hashtable<SetWidget, SetWidget, SWHash, std::equal_to<SetWidget>, 37, true, true>;
template class std::intrusive_hashtable<int, MapWidget, std::hash<int>, std::equal_to<int>, 37, false, true>;

template class std::intrusive_hash_set<SetWidget, 37, SWHash>;
template class std::intrusive_hash_multiset<SetWidget, 37, SWHash>;

template class std::intrusive_hash_map<int, MapWidget, 37>;
template class std::intrusive_hash_multimap<int, MapWidget, 37>;

template class std::intrusive_hash_set<IHWidget, 37, IHWHash>;
template class std::intrusive_hash_multiset<IHWidget, 37, IHWHash>;

template class std::intrusive_hash_map<int, IHWidget, 37, IHWHash>;
template class std::intrusive_hash_multimap<int, IHWidget, 37, IHWHash>;
...
int TestIntrusiveHash()
{
...

	{
		...
		//itf = ihmSW1.find_as(SetWidget(7)); // Can't work unless there was a default std::hash function for SetWidget.
		//VERIFY(itf->mX == 7);
		...
		itf = ihmSW1.find_as(SetWidgetComparable(7), SWCHash(), std::equal_to_2<SetWidget, SetWidgetComparable>());
		VERIFY(itf->mX == 7);

		itfc = ihmSW1Const.find_as(SetWidgetComparable(7), SWCHash(), std::equal_to_2<SetWidget, SetWidgetComparable>());
		VERIFY(itfc->mX == 7);
		...
		std::pair<IHM_SW::iterator, IHM_SW::iterator> p = ihmSW1.equal_range(SetWidget(1));
		VERIFY(p.first != ihmSW1.end());
		VERIFY(p.second != ihmSW1.end());

		std::pair<IHM_SW::const_iterator, IHM_SW::const_iterator> pc = ihmSW1Const.equal_range(SetWidget(1));
...

	{
		...
		itf = ihmMW1.find_as(7.f, std::hash<float>(), std::equal_to_2<int, float>());
		VERIFY(itf->mKey == 7);

		itfc = ihmMW1Const.find_as(7.f, std::hash<float>(), std::equal_to_2<int, float>());
		VERIFY(itfc->mKey == 7);
		...
		std::pair<IHM_MW::iterator, IHM_MW::iterator> p = ihmMW1.equal_range(1);
		VERIFY(p.first != ihmMW1.end());
		VERIFY(p.second != ihmMW1.end());

		std::pair<IHM_MW::const_iterator, IHM_MW::const_iterator> pc = ihmMW1Const.equal_range(1);
...

	{
		...
		std::intrusive_hash_set<SetWidget, 1, SWHash> hs;
...

	{
		...
		std::intrusive_hash_set<SetWidget, 37, SWHash> hs;
		SetWidget node1, node2, node3;
