...
// TestMapConstruction
//
// This test compares std::map/multimap to std::map/multimap. It could possibly
// work for comparing std::hash_map to C++11 std::unordered_map, but we would 
...
int TestMapConstruction()
{
...

	{
		...
		std::scoped_ptr<T1> pt1A(new T1);
		std::scoped_ptr<T2> pt2A(new T2);
		T1& t1A = *pt1A;
		T2& t2A = *pt2A;
		nErrorCount += CompareContainers(t1A, t2A, "Map ctor", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());
		EATEST_VERIFY(t1A.validate());


		std::scoped_ptr<T1> pt1B(new T1);
		std::scoped_ptr<T2> pt2B(new T2);
		T1& t1B = *pt1B;
		T2& t2B = *pt2B;
		nErrorCount += CompareContainers(t1B, t2B, "Map ctor", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());


		std::scoped_ptr<T1> pt1C(new T1);
		std::scoped_ptr<T2> pt2C(new T2);
		T1& t1C = *pt1C;
		T2& t2C = *pt2C;
		for(int i = 0; i < 1000; i++)
		{
			...
			nErrorCount += CompareContainers(t1C, t2C, "Map insert", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());
		}


		std::scoped_ptr<T1> pt1D(new T1);
		std::scoped_ptr<T2> pt2D(new T2);
		T1& t1D = *pt1D;
		T2& t2D = *pt2D;
		nErrorCount += CompareContainers(t1D, t2D, "Map ctor", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());


		std::scoped_ptr<T1> pt1E(new T1(t1C));
		std::scoped_ptr<T2> pt2E(new T2(t2C));
		T1& t1E = *pt1E;
		T2& t2E = *pt2E;
		EATEST_VERIFY(t1E.validate());
		nErrorCount += CompareContainers(t1E, t2E, "Map ctor", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());


		std::scoped_ptr<T1> pt1F(new T1(t1C.begin(), t1C.end()));
		std::scoped_ptr<T2> pt2F(new T2(t2C.begin(), t2C.end()));
		T1& t1F = *pt1F;
		T2& t2F = *pt2F;
		EATEST_VERIFY(t1F.validate());
		nErrorCount += CompareContainers(t1F, t2F, "Map ctor", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());


		// operator=(const map&)
		t1E = t1D;
		t2E = t2D;
		nErrorCount += CompareContainers(t1D, t2D, "Map operator=", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());
		nErrorCount += CompareContainers(t1E, t2E, "Map operator=", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());


		// operator=(map&&)
		// We test just the EASTL container here.
		std::scoped_ptr<T1> pT1P(new T1);
		std::scoped_ptr<T1> pT1Q(new T1);
		T1& t1P = *pT1P;
		T1& t1Q = *pT1Q;
	  ...
	  //EATEST_VERIFY((t1P.size() == 3) && (t1P.find(k13) != t1P.end()) && (t1P.find(k14) != t1P.end()) && (t1P.find(k15) != t1P.end()));  // Currently operator=(this_type&& x) clears x instead of swapping with it.
		...
		EATEST_VERIFY(t1E.validate());
		nErrorCount += CompareContainers(t1D, t2D, "Map swap", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());
		nErrorCount += CompareContainers(t1E, t2E, "Map swap", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());
		...
		t2A.clear();
		EATEST_VERIFY(t1A.validate());
		nErrorCount += CompareContainers(t1A, t2A, "Map clear", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());

		t1B.clear();
		t2B.clear();
		EATEST_VERIFY(t1B.validate());
		nErrorCount += CompareContainers(t1B, t2B, "Map clear", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());
...
int TestMapMutation()
{
	...
	{
		std::scoped_ptr<T1> pt1A(new T1); // We use a pointers instead of concrete object because it's size may be huge.
		std::scoped_ptr<T2> pt2A(new T2);
		T1&                   t1A = *pt1A;
		T2&                   t2A = *pt2A;
		int                   i, iEnd, p;
		...
		if(gEASTL_TestLevel >= kEASTL_TestLevelLow)
		{
			...
			for(p = 0; p < gEASTL_TestLevel * 100; p++) // For each permutation...
			{
				...
				for(i = 0, iEnd = (int)valueArrayInsert.size(); i < iEnd; i++)
				{
					...
					EATEST_VERIFY(t1A.validate());
					nErrorCount += CompareContainers(t1A, t2A, "Map insert", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());
				}
				...
				for(i = 0, iEnd = (int)valueArrayInsert.size(); i < iEnd; i++)
				{
					...
					EATEST_VERIFY(n1 == n2);
					EATEST_VERIFY(t1A.validate());
					nErrorCount += CompareContainers(t1A, t2A, "Map erase", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());
				}
	...
	{  // Other insert and erase operations
			...
			EATEST_VERIFY(t1A.validate());
			nErrorCount += CompareContainers(t1A, t2A, "Map erase(first, last)", std::use_first<typename T1::value_type>(), std::use_first<typename T2::value_type>());
...
int TestMapCpp11()
{
	...
	toMapIterator = toMap.insert(toMapInsertResult.first, std::move(value91));
	...
	toMapIterator = toMap.insert(toMap.begin(), std::move(value10)); // specify a bad hint. Insertion should still work.
	...
	toMapInsertResult = toMap.emplace(12, std::move(to12));
	EATEST_VERIFY(toMapInsertResult.second == true);
	EATEST_VERIFY(toMapInsertResult.first->first == 12);
	EATEST_VERIFY(toMap.find(12) != toMap.end());

	TestObject to121(121);
	toMapInsertResult = toMap.emplace(12, std::move(to121));
	EATEST_VERIFY(toMapInsertResult.second == false);
	EATEST_VERIFY(toMapInsertResult.first->first == 12);
	...
	toMapInsertResult = toMap.emplace(std::piecewise_construct, std::make_tuple(13), std::make_tuple(1, 2, 10)); // 1 + 2 + 10 = 13
	EATEST_VERIFY(toMapInsertResult.second == true);
	EATEST_VERIFY(toMapInsertResult.first->first == 13);
	EATEST_VERIFY(toMap.find(13) != toMap.end());

	toMapInsertResult = toMap.emplace(std::piecewise_construct, std::make_tuple(13), std::make_tuple(1, 30, 100)); // 1 + 30 + 100 = 131
	EATEST_VERIFY(toMapInsertResult.second == false);
	EATEST_VERIFY(toMapInsertResult.first->first == 13);
	...
	toMapInsertResult = toMap.emplace(16, std::move(to16));
	EATEST_VERIFY(toMapInsertResult.second == true);
	EATEST_VERIFY(toMap.find(16) != toMap.end());

	TestObject to161(161);
	toMapIterator = toMap.emplace_hint(toMapInsertResult.first, 16, std::move(to161));
	...
	toMapIterator = toMap.emplace_hint(toMap.begin(), 17, std::move(to17)); // specify a bad hint. Insertion should still work.
	EATEST_VERIFY(toMapIterator->first == 17);
	EATEST_VERIFY(toMap.find(17) != toMap.end());

	EATEST_VERIFY(toMap.find(18) == toMap.end());
	toMapInsertResult = toMap.emplace(std::piecewise_construct, std::make_tuple(18), std::make_tuple(3, 5, 10)); // 3 + 5 + 10 = 18
	EATEST_VERIFY(toMapInsertResult.second == true);
	EATEST_VERIFY(toMap.find(18) != toMap.end());

	toMapIterator = toMap.emplace_hint(toMapInsertResult.first, std::piecewise_construct, std::make_tuple(18), std::make_tuple(1, 80, 100)); // 1 + 80 + 100 = 181
	EATEST_VERIFY(toMapIterator->first == 18);
	EATEST_VERIFY(toMapIterator->second.mX == 18);
	EATEST_VERIFY(toMap.find(18) != toMap.end());

	EATEST_VERIFY(toMap.find(19) == toMap.end());
	toMapIterator = toMap.emplace_hint(toMap.begin(), std::piecewise_construct, std::make_tuple(19), std::make_tuple(4, 5, 10)); // 4 + 5 + 10 = 19 // specify a bad hint. Insertion should still work.
...
int TestMultimapCpp11()
{
	...
	value_type value91(9, TestObject(91));
	toMapIterator = toMap.insert(toMapIterator, std::move(value91));
	EATEST_VERIFY(toMapIterator->first == 9);
	...
	TestObject to12(12);
	EATEST_VERIFY(toMap.find(12) == toMap.end());
	toMapIterator = toMap.emplace(12, std::move(to12));
	EATEST_VERIFY(toMapIterator->first == 12);
	EATEST_VERIFY(toMap.find(12) != toMap.end());

	TestObject to121(121);
	toMapIterator = toMap.emplace(12, std::move(to121));
	EATEST_VERIFY(toMapIterator->first == 12);
	EATEST_VERIFY(toMapIterator->second.mX == 121);
	EATEST_VERIFY(toMap.count(12) == 2);

	EATEST_VERIFY(toMap.find(13) == toMap.end());
	toMapIterator = toMap.emplace(std::piecewise_construct, std::make_tuple(13), std::make_tuple(1, 2, 10)); // 1 + 2 + 10 = 13
	EATEST_VERIFY(toMapIterator->first == 13);
	EATEST_VERIFY(toMap.find(13) != toMap.end());

	toMapIterator = toMap.emplace(std::piecewise_construct, std::make_tuple(13), std::make_tuple(1, 30, 100)); // 1 + 30 + 100 = 131
	EATEST_VERIFY(toMapIterator->first == 13);
	...
	TestObject to16(16);
	EATEST_VERIFY(toMap.find(16) == toMap.end());
	toMapIterator = toMap.emplace(16, std::move(to16));
	EATEST_VERIFY(toMap.find(16) != toMap.end());

	TestObject to161(161);
	toMapIterator = toMap.emplace_hint(toMapIterator, 16, std::move(to161));
	EATEST_VERIFY(toMapIterator->first == 16);
	...
	EATEST_VERIFY(toMap.find(18) == toMap.end());
	toMapIterator = toMap.emplace(std::piecewise_construct, std::make_tuple(18), std::make_tuple(3, 5, 10)); // 3 + 5 + 10 = 18
	EATEST_VERIFY(toMap.find(18) != toMap.end());

	toMapIterator = toMap.emplace_hint(toMapIterator, std::piecewise_construct, std::make_tuple(18), std::make_tuple(1, 80, 100)); // 1 + 80 + 100 = 181
	EATEST_VERIFY(toMapIterator->first == 18);
