...
static int TestFixedAllocator()
{
	...
	{
		// fixed_pool_reference
		typedef std::list<int, fixed_pool_reference> WidgetList;
	 
		WidgetList::node_type buffer[16];
		std::fixed_pool myPool(buffer, sizeof(buffer), sizeof(WidgetList::node_type), 16);
...
static int TestAllocatorMalloc()
{
	...
	{
		typedef std::list<int, std::allocator_malloc> WidgetList;
...
static int TestCoreAllocatorAdapter()
{
	...
	std::list<TestClass, Adapter> widgetList(Adapter("UI/WidgetList", &gEASTLTestCoreAllocator));
	widgetList.push_back(TestClass());
	EATEST_VERIFY(widgetList.size() == 1);

	std::vector<TestClass, Adapter> widgetVector(100, Adapter("UI/WidgetVector", &gEASTLTestCoreAllocator));
	widgetVector.push_back(TestClass());
	EATEST_VERIFY(widgetVector.size() == 101);

	std::vector<TestClass, Adapter> widgetVector2(widgetVector);
