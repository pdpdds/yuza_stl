...
template class std::fixed_list<int, 1, true,  EASTLAllocatorType>;
template class std::fixed_list<int, 1, false, EASTLAllocatorType>;
...
{
	std::fixed_list<StructWithContainerOfStructs,4> children;
};
*/
...
int TestFixedList()
{
...

	{
		...
		list0402.sort(std::less<int>());
		VERIFY(list0402 == list0403);

		list0401.merge(list0402, std::less<int>());
		list0404.sort();
...

	{
		...
		// void splice(iterator position, this_type& x);
		it = listA.begin(); std::advance(it, 2);
		...
		// void splice(iterator position, this_type& x, iterator i);
		it = listA.begin(); std::advance(it, 6);
...

	{
		...
		fixed_list<TestObject, 16> toListA(std::move(list3TO33));
		...
		fixed_list<TestObject, 16, true, MallocAllocator> toListB(std::move(list4TO44), MallocAllocator());
	...
	{   // Regression of user test
		struct Dummy
		{
		   typedef std::fixed_list<FixedListTest::Item, 10, false> TCollection;
