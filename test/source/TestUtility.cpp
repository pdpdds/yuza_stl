...
// One-off tests and regressions
		...
		// template<typename U>
		// pair(U&& x, const T2& y)
		typedef std::pair<uint16_t, const char8_t*> LCIDMapping;
		...
		// template<typename V>
		// pair(const T1& x, V&& y)
		typedef std::pair<const char8_t*, uint16_t> LCIDMapping2;
...
//       an entity of type "void *" second(std::forward<V>(v)) {}
// template <typename U, typename V>
// pair(U&& u, V&& v);
#if EASTL_PAIR_CONFORMANCE
		typedef std::pair<float*, void*> TestPair1;
		float fOne = 1.f;
	...
	{ // reported user-regression structured binding unpacking for iterators
		...
		auto t = std::make_pair(v.begin(), v.end() - 1);
		auto [x,y] = t;
		EATEST_VERIFY(*x == 1);
		EATEST_VERIFY(*y == 6);
	}
	...
	{ // reported user-regression structured binding unpacking for iterators
		std::vector<int> v = {1,2,3,4,5,6};
		auto t = std::make_pair(v.begin(), v.end());
		auto [x,y] = t;
		EATEST_VERIFY(*x == 1);
		EA_UNUSED(y);
	}
	...
	{ // reported user-regression for const structured binding unpacking for iterators
		std::vector<int> v = {1,2,3,4,5,6};
		const auto [x,y] = std::make_pair(v.begin(), v.end());;
		EATEST_VERIFY(*x == 1);
		EA_UNUSED(y);
	}
#endif
...
static int TestUtilityRelops()
{
	...
	{
		using namespace std::rel_ops;  // Defines default versions of operators !=, <, >, <=, >= based on == and <.
...
// is_nothrow_swappable
	...
	static_assert((std::is_swappable<int>::value == true), "is_swappable failure");
	static_assert((std::is_swappable<std::vector<int> >::value == true), "is_swappable failure");
	static_assert((std::is_swappable<ThrowSwappable>::value == true), "is_swappable failure");
	#if EASTL_VARIABLE_TEMPLATES_ENABLED
		static_assert((std::is_swappable_v<int> == true), "is_swappable failure");
		static_assert((std::is_swappable_v<std::vector<int> > == true), "is_swappable failure");
		static_assert((std::is_swappable_v<ThrowSwappable> == true), "is_swappable failure");
	#endif
// Need to come up with a class that's not swappable. How do we do that, given the universal swap template?  
// static_assert((std::is_swappable<?>::value  == false), "is_swappable failure");
#endif

#if EASTL_TYPE_TRAIT_is_nothrow_swappable_CONFORMANCE
	static_assert((std::is_nothrow_swappable<int>::value == true), "is_nothrow_swappable failure");  // There currently isn't any specialization for swap of scalar types that's nothrow.
	static_assert((std::is_nothrow_swappable<std::vector<int> >::value == false), "is_nothrow_swappable failure");
	static_assert((std::is_nothrow_swappable<ThrowSwappable>::value == false), "is_nothrow_swappable failure");
	static_assert((std::is_nothrow_swappable<NoThrowSwappable>::value == true), "is_nothrow_swappable failure");
	#if EASTL_VARIABLE_TEMPLATES_ENABLED
		static_assert((std::is_nothrow_swappable_v<int> == true), "is_nothrow_swappable failure");  // There currently isn't any specialization for swap of scalar types that's nothrow.
		static_assert((std::is_nothrow_swappable_v<std::vector<int>> == false), "is_nothrow_swappable failure");
		static_assert((std::is_nothrow_swappable_v<ThrowSwappable> == false), "is_nothrow_swappable failure");
		static_assert((std::is_nothrow_swappable_v<NoThrowSwappable> == true), "is_nothrow_swappable failure");
	#endif
#endif
...
// is_nothrow_swappable_with
	static_assert(std::is_swappable_with<int&, int&>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<int, int>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<int&, int>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<int, int&>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<int, short>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<int, long>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<int, std::vector<int>>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<void, void>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<int, void>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<void, int>::value, "is_swappable_with failure");
	static_assert(!std::is_swappable_with<ThrowSwappable, ThrowSwappable>::value, "is_swappable_with failure");
	static_assert(std::is_swappable_with<ThrowSwappable&, ThrowSwappable&>::value, "is_swappable_with failure");
	static_assert(std::is_swappable_with<Swappable1&, Swappable1&>::value, "is_swappable_with failure");
	static_assert(std::is_swappable_with<Swappable1&, Swappable2&>::value, "is_swappable_with failure");
	static_assert(std::is_swappable_with<Swappable2&, Swappable1&>::value, "is_swappable_with failure");

    #if EASTL_VARIABLE_TEMPLATES_ENABLED
		static_assert(std::is_swappable_with_v<int&, int&>, "is_swappable_with_v failure");
		static_assert(!std::is_swappable_with_v<int, int>, "is_swappable_with_v failure");
		static_assert(!std::is_swappable_with_v<int&, int>, "is_swappable_with_v failure");
		static_assert(!std::is_swappable_with_v<int, int&>, "is_swappable_with_v failure");
		static_assert(!std::is_swappable_with_v<int, short>, "is_swappable_with_v failure");
		static_assert(!std::is_swappable_with_v<int, long>, "is_swappable_with_v failure");
		static_assert(!std::is_swappable_with_v<int, std::vector<int>>, "is_swappable_with_v failure");
	    static_assert(!std::is_swappable_with_v<void, void>, "is_swappable_with_v failure");
	    static_assert(!std::is_swappable_with_v<int, void>, "is_swappable_with_v failure");
	    static_assert(!std::is_swappable_with_v<void, int>, "is_swappable_with_v failure");
		static_assert(!std::is_swappable_with_v<ThrowSwappable, ThrowSwappable>, "is_swappable_with_v failure");
		static_assert(std::is_swappable_with_v<ThrowSwappable&, ThrowSwappable&>, "is_swappable_with_v failure");
		static_assert(std::is_swappable_with_v<Swappable1&, Swappable1&>, "is_swappable_with_v failure");
		static_assert(std::is_swappable_with_v<Swappable1&, Swappable2&>, "is_swappable_with_v failure");
		static_assert(std::is_swappable_with_v<Swappable2&, Swappable1&>, "is_swappable_with_v failure");
    #endif // EASTL_VARIABLE_TEMPLATES_ENABLED

#if EASTL_TYPE_TRAIT_is_nothrow_swappable_with_CONFORMANCE
	static_assert(std::is_nothrow_swappable_with<int&, int&>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<int, int>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<int&, int>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<int, int&>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<int, short>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<int, long>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<int, std::vector<int>>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<void, void>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<int, void>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<void, int>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<ThrowSwappable, ThrowSwappable>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<ThrowSwappable&, ThrowSwappable&>::value, "is_nothrow_swappable_with failure");
	static_assert(!std::is_nothrow_swappable_with<NoThrowSwappable, NoThrowSwappable>::value, "is_nothrow_swappable_with failure");
	static_assert(std::is_nothrow_swappable_with<NoThrowSwappable&, NoThrowSwappable&>::value, "is_nothrow_swappable_with failure");

    #if EASTL_VARIABLE_TEMPLATES_ENABLED
		static_assert(std::is_nothrow_swappable_with_v<int&, int&>, "is_nothrow_swappable_with_v failure");
		static_assert(!std::is_nothrow_swappable_with_v<int, int>, "is_nothrow_swappable_with_v failure");
		static_assert(!std::is_nothrow_swappable_with_v<int&, int>, "is_nothrow_swappable_with_v failure");
		static_assert(!std::is_nothrow_swappable_with_v<int, int&>, "is_nothrow_swappable_with_v failure");
		static_assert(!std::is_nothrow_swappable_with_v<int, short>, "is_nothrow_swappable_with_v failure");
		static_assert(!std::is_nothrow_swappable_with_v<int, long>, "is_nothrow_swappable_with_v failure");
		static_assert(!std::is_nothrow_swappable_with_v<int, std::vector<int>>, "is_nothrow_swappable_with_v failure");
	    static_assert(!std::is_nothrow_swappable_with_v<void, void>, "is_nothrow_swappable_with_v failure");
	    static_assert(!std::is_nothrow_swappable_with_v<int, void>, "is_nothrow_swappable_with_v failure");
	    static_assert(!std::is_nothrow_swappable_with_v<void, int>, "is_nothrow_swappable_with_v failure");
		static_assert(!std::is_nothrow_swappable_with_v<ThrowSwappable, ThrowSwappable>, "is_nothrow_swappable_with_v failure");
		static_assert(!std::is_nothrow_swappable_with_v<ThrowSwappable&, ThrowSwappable&>, "is_nothrow_swappable_with_v failure");
		static_assert(!std::is_nothrow_swappable_with_v<NoThrowSwappable, NoThrowSwappable>, "is_nothrow_swappable_with_v failure");
		static_assert(std::is_nothrow_swappable_with_v<NoThrowSwappable&, NoThrowSwappable&>, "is_nothrow_swappable_with_v failure");
    #endif // EASTL_VARIABLE_TEMPLATES_ENABLED
#endif
...
// move_if_noexcept
	...
	noexcept_move_copy nemcB =
		std::move_if_noexcept(nemcA);  // nemcB should be constructed via noexcept_move_copy(noexcept_move_copy&&)
	EATEST_VERIFY(nemcA.mStatus == false);
	EA_UNUSED(nemcB);

	noexcept_move_no_copy nemncA;
	noexcept_move_no_copy nemncB = std::move_if_noexcept(
	...
	except_move_copy emcB = std::move_if_noexcept(
	...
	except_move_no_copy emncB =
		std::move_if_noexcept(emncA);  // emncB should be constructed via except_move_no_copy(except_move_no_copy&&)
...
static int TestUtilityExchange()
{
...

	{
		...
		auto r = std::exchange(b, true);
		EATEST_VERIFY(r);

		r = std::exchange(b, false);
		EATEST_VERIFY(r);
		EATEST_VERIFY(!b);

		r = std::exchange(b, true);
		EATEST_VERIFY(!r);
		EATEST_VERIFY(b);
	}

	{
		...
		TestObject a(42);
		auto r = std::exchange(a, TestObject(24));
...

	{
		...
		std::string a(pElectronicArts);
		auto r = std::exchange(a, pEAVancouver);

		EATEST_VERIFY(r == pElectronicArts);
		EATEST_VERIFY(a == pEAVancouver);

		r = std::exchange(a, "EA Standard Template Library");
	...
	// Construct pair using single move constructor
	{
		struct TestPairSingleMoveConstructor
		{
			void test(int&& val)
			{
				std::pair<int,int> p(std::pair_first_construct, std::move(val));
			}
		};

		int i1 = 1;
		TestPairSingleMoveConstructor test;
		test.test(std::move(i1));
	}
	...
	// pair(T&&)      -> pair(const U& &&)      -> pair(const U&)
	{
		...
		using VectorOfPairWithReference = std::vector<std::pair<const FooType&, float>>;
