...
template class std::set<TestObject>;
template class std::multiset<TestObject>;


///////////////////////////////////////////////////////////////////////////////
// typedefs
//
typedef std::set<int> VS1;
typedef std::set<TestObject> VS4;
typedef std::multiset<int> VMS1;
typedef std::multiset<TestObject> VMS4;
...
int TestSet()
{
...

	{
		...
		nErrorCount += TestSetCpp11<std::set<TestObject> >();

		nErrorCount += TestMultisetCpp11<std::multiset<TestObject> >();
	}
	...
	{ // set erase_if tests
		set<int> s = {0, 1, 2, 3, 4};
		std::erase_if(s, [](auto i) { return i % 2 == 0;});
...

	{
		...
		// moved from during the std::set construction process.
		std::vector<xvalue_test> m1 = {{0}, {1}, {2}, {3}, {4}, {5}};
		std::set<xvalue_test> m2{m1.begin(), m1.end()};

		bool result = std::all_of(m1.begin(), m1.end(), 
...

	{
		...
		// std::set construction process when using an std::move_iterator.
		std::vector<xvalue_test> m1 = {{0}, {1}, {2}, {3}, {4}, {5}};
		std::set<xvalue_test> m2{std::make_move_iterator(m1.begin()), std::make_move_iterator(m1.end())};
