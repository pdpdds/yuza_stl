...
// TestSetConstruction
//
// This test compares std::set/multiset to std::set/multiset. It could possibly
// work for comparing std::hash_set to C++11 std::unordered_set, but we would 
...
int TestSetConstruction()
{
...

	{
		...
		T1& t1A = *pt1A;
		T2& t2A = *pt2A;
		nErrorCount += CompareContainers(t1A, t2A, "Set ctor", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		VERIFY(t1A.validate());
		...
		T1& t1B = *pt1B;
		T2& t2B = *pt2B;
		nErrorCount += CompareContainers(t1B, t2B, "Set ctor", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		...
		for(int i = 0; i < 1000; i++)
		{
			...
			VERIFY(t1C.validate());
			nErrorCount += CompareContainers(t1C, t2C, "Set insert", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		}
		...
		T1& t1D = *pt1D;
		T2& t2D = *pt2D;
		nErrorCount += CompareContainers(t1D, t2D, "Set ctor", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		...
		T2& t2E = *pt2E;
		VERIFY(t1E.validate());
		nErrorCount += CompareContainers(t1E, t2E, "Set ctor", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		...
		T2& t2F = *pt2F;
		VERIFY(t1F.validate());
		nErrorCount += CompareContainers(t1F, t2F, "Set ctor", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());


		// operator=
		t1E = t1D;
		t2E = t2D;
		nErrorCount += CompareContainers(t1D, t2D, "Set operator=", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		nErrorCount += CompareContainers(t1E, t2E, "Set operator=", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());


		// operator=(set&&)
		// We test just the EASTL container here.
		std::scoped_ptr<T1> pT1P(new T1); // We use a pointers instead of concrete object because it's size may be huge.
	  ...
	  //EATEST_VERIFY((t1P.size() == 3) && (t1P.find(v13) != t1P.end()) && (t1P.find(v14) != t1P.end()) && (t1P.find(v15) != t1P.end()));  // Currently operator=(this_type&& x) clears x instead of swapping with it.
		...
		VERIFY(t1D.validate());
		VERIFY(t1E.validate());
		nErrorCount += CompareContainers(t1D, t2D, "Set swap", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		nErrorCount += CompareContainers(t1E, t2E, "Set swap", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		...
		VERIFY(t1D.validate());
		VERIFY(t1E.validate());
		nErrorCount += CompareContainers(t1D, t2D, "Global swap", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		nErrorCount += CompareContainers(t1E, t2E, "Global swap", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
		...
		t2A.clear();
		VERIFY(t1A.validate());
		nErrorCount += CompareContainers(t1A, t2A, "Set clear", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());

		t1B.clear();
		t2B.clear();
		VERIFY(t1B.validate());
		nErrorCount += CompareContainers(t1B, t2B, "Set clear", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
...
int TestSetMutation()
{
		...
		if(gEASTL_TestLevel >= kEASTL_TestLevelLow)
		{
			...
			for(p = 0; p < gEASTL_TestLevel * 100; p++) // For each permutation...
			{
				...
				for(i = 0, iEnd = (int)valueArrayInsert.size(); i < iEnd; i++)
				{
					...
					VERIFY(t1A.validate());
					nErrorCount += CompareContainers(t1A, t2A, "Set insert", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
				}
				...
				for(i = 0, iEnd = (int)valueArrayInsert.size(); i < iEnd; i++)
				{
					...
					VERIFY(n1 == n2);
					VERIFY(t1A.validate());
					nErrorCount += CompareContainers(t1A, t2A, "Set erase", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
				}
	...
	{  // Other insert and erase operations
		...
		VERIFY(t1A.validate());
		nErrorCount += CompareContainers(t1A, t2A, "Set erase(first, last)", std::use_self<typename T1::value_type>(), std::use_self<typename T2::value_type>());
