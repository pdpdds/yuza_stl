...
#include <EASTL/hash_map>
#include <EASTL/unordered_set.h>
#include <EASTL/unordered_map>
#include <EASTL/map>
...
struct HashTest
{
	...
	{
		return std::hash<HashType>{}(std::forward<Args>(args)...);
	}
};
...
template class std::hashtable<int,
                                std::pair<const int, int>,
                                std::allocator,
                                std::use_first<std::pair<const int, int>>,
                                std::equal_to<int>,
                                std::hash<int>,
                                mod_range_hashing,
                                default_ranged_hash,
...
template class std::hash_set<int>;
template class std::hash_multiset<int>;
template class std::hash_map<int, int>;
template class std::hash_multimap<int, int>;
template class std::hash_set<Align32>;
template class std::hash_multiset<Align32>;
template class std::hash_map<Align32, Align32>;
template class std::hash_multimap<Align32, Align32>;

// validate static assumptions about hashtable core types
typedef std::hash_node<int, false> HashNode1;
typedef std::hash_node<int, true> HashNode2;
...
{   
...

	{
		...
		nErrorCount += TestMapCpp11<std::hash_map<int, TestObject>>();
		nErrorCount += TestMapCpp11<std::unordered_map<int, TestObject>>();

		nErrorCount += TestSetCpp11<std::hash_set<TestObject>>();
		nErrorCount += TestSetCpp11<std::unordered_set<TestObject>>();

		nErrorCount += TestMultimapCpp11<std::hash_multimap<int, TestObject>>();
		nErrorCount += TestMultimapCpp11<std::unordered_multimap<int, TestObject>>();

		nErrorCount += TestMultisetCpp11<std::hash_multiset<TestObject>>();
		nErrorCount += TestMultisetCpp11<std::unordered_multiset<TestObject>>();

		nErrorCount += TestMapCpp11NonCopyable<std::hash_map<int, NonCopyable>>();
		nErrorCount += TestMapCpp11NonCopyable<std::unordered_map<int, NonCopyable>>();
	}

	{
		// C++17 try_emplace and related functionality
		nErrorCount += TestMapCpp17<std::hash_map<int, TestObject>>();
		nErrorCount += TestMapCpp17<std::unordered_map<int, TestObject>>();
	}
	...
	{ // hash_set erase_if
		...
		std::erase_if(m, [](auto i) { return i % 2 == 0; });
	...
	{
		typedef std::hash_multiset<int> HashMultisetInt;
	...
	{ 
		...
		typedef std::hash_multimap<uint32_t, uint32_t*> Map;
		Map* pMap = new Map;
		delete pMap;
	}
	...
	{ 
		...
		std::hash_map<int, std::string*>* _hmTextureList;
		_hmTextureList = new std::hash_map<int, std::string*>();
		std::string* a = NULL;
		(*_hmTextureList)[0] = a;
...

	{
		...
		typedef std::hash_multimap<HashRegressionA*, HashRegressionB> HMM;
		HMM m_hash;
...

	{
		...
		typedef std::hash_map<int64_t, Struct> AuditByBlazeIdMap;
	...
	#if !defined(EA_COMPILER_MSVC_2013)
	{
		...
		std::unordered_set<Movable, Movable::Hash> a, b;
		swap(a,b);
	}
	#endif
...

	{
		...
		using StringStringHashMap = std::hash_map<String, String, std::string_hash<String>, std::equal_to<String>, AllocatorType>;
	...
	
	{
...

		{
			...
			std::hash_map<std::pair<int, const char*>, bool, std::hash<std::pair<int, const char*>>, name_equals, std::allocator> m_TempNames;
