...
template class std::fixed_slist<int, 1, true,  EASTLAllocatorType>;
template class std::fixed_slist<int, 1, false, EASTLAllocatorType>;
...
{
	std::fixed_slist<StructWithContainerOfStructs,4> children;
};
*/
...
int TestFixedSList()
{
...

	{
		...
		fixed_slist<TestObject, 16> toListA(std::move(slist3TO33));
		...
		fixed_slist<TestObject, 16, true, MallocAllocator> toListB(std::move(slist4TO44), MallocAllocator());
