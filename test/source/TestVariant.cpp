...
int TestVariantBasic()
{
...

	{
		...
		static_assert(!std::is_trivially_destructible_v<MyObj>, "MyObj can't be trivially destructible");
		static_assert(!std::is_trivially_destructible_v<MyObj2>, "MyObj2 can't be trivially destructible");

		{
			std::variant<MyObj, MyObj2> myVar;
			VERIFY(get<MyObj>(myVar).i == 1337);
		}

		{
			std::variant<MyObj, MyObj2> myVar = MyObj();
			VERIFY(get<MyObj>(myVar).i == 1337);
		}

		{
			int i = 42;
			std::variant<MyObj, MyObj2> myVar = MyObj2(i);
...

		{
			...
			std::variant<MyObj, MyObj2> myVar = m;
	...
	{
		{
			...
			static_assert(std::is_default_constructible_v<v_t>, "valueless_by_exception error");
...

		{
			...
			static_assert(!std::is_default_constructible<NotDefaultConstructibleButHasConversionCtor>::value, "valueless_by_exception error");
...
int TestVariantVisitor()
{
	...
	//     for (auto& e : arr)
	//     {
	//         std::visit(overloaded{[&](int)    { count++; },
	//                                 [&](string) { count++; },
	//                                 [&](double) { count++; },
	//                                 [&](long)   { count++; }}, e);
	//     }
...
void TestCompilation(const double e) { std::variant<double> v{e}; }
...
int TestVariantUserRegressionCopyMoveAssignmentOperatorLeak()
{
	...
	{
		{
			std::variant<TestObject> v = TestObject(1337);
			VERIFY(std::get<TestObject>(v).mX == 1337);
			std::variant<TestObject> v2 = TestObject(1338);
			VERIFY(std::get<TestObject>(v2).mX == 1338);
			v.operator=(v2);
			VERIFY(std::get<TestObject>(v).mX == 1338);
			VERIFY(std::get<TestObject>(v2).mX == 1338);
		}
		VERIFY(TestObject::IsClear());
		TestObject::Reset();
	...
	{
		{
			...
			VERIFY(std::get<TestObject>(v).mX == 1337);
			v = {};
			VERIFY(std::get<TestObject>(v).mX == 0);
		}
		VERIFY(TestObject::IsClear());
		TestObject::Reset();
	}
