...
template class std::ring_buffer< int,        std::vector<int>        >;
template class std::ring_buffer< Align64,    std::vector<Align64>    >;
template class std::ring_buffer< TestObject, std::vector<TestObject> >;

template class std::ring_buffer< int,        std::deque<int>        >;
template class std::ring_buffer< Align64,    std::deque<Align64>    >;
template class std::ring_buffer< TestObject, std::deque<TestObject> >;

template class std::ring_buffer< int,        std::list<int>        >;
template class std::ring_buffer< Align64,    std::list<Align64>    >;
template class std::ring_buffer< TestObject, std::list<TestObject> >;

// TODO(rparolin):  To consider adding support for std::array. 
// template class std::ring_buffer< int, std::array<int, 64>>;

typedef std::fixed_string<char, 256, false>          RBFixedString;
typedef std::fixed_vector<RBFixedString, 100, false> RBFixedStringVector;
typedef RBFixedStringVector::overflow_allocator_type   RBFixedStringVectorOverflowAllocator;
template class std::ring_buffer<RBFixedString, RBFixedStringVector, RBFixedStringVectorOverflowAllocator>;

typedef std::fixed_vector<int, 100, false> RBFixedIntVector;
template class std::ring_buffer<int, RBFixedIntVector, RBFixedIntVector::overflow_allocator_type>;
// template class std::ring_buffer<int, RBFixedIntVector>;  // currently fails to compile

typedef std::fixed_vector<int, 100> RBFixedIntVectorWithOverFlow;
template class std::ring_buffer<int, RBFixedIntVectorWithOverFlow, RBFixedIntVectorWithOverFlow::overflow_allocator_type>; 
// template class std::ring_buffer<int, RBFixedIntVectorWithOverFlow>; // currently fails to compile
...
int TestRingBuffer()
{
		...
		*it = counterBuffer;
		...
		for(int j = 0; j < 10000 + (gEASTL_TestLevel * 10000); j++)
		{
			...
			if(op == 0)
			{
				...
				rbVectorString.push_back(string(counterBuffer));
				EATEST_VERIFY(rbVectorString.size() == std::min(s + 1, rbVectorString.capacity()));
			}
			else if(op == 1)
			{
				// void push_back();
			...
			else if(op == 7)
			{
				...
				EATEST_VERIFY(rbVectorString.size() == std::min(s + count, rbVectorString.capacity()));
			}
			else if(op == 8)
			{
		...
		std::swap(rbVectorString2, rbVectorString4);
		EATEST_VERIFY(rbVectorString2.validate());
		EATEST_VERIFY(rbVectorString3.validate());
		EATEST_VERIFY(rbVectorString4.validate());
		...
		*it = counterBuffer;
		...
		for(int j = 0; j < 10000 + (gEASTL_TestLevel * 10000); j++)
		{
			...
			if(op == 0)
			{
				...
				rbListString.push_back(string(counterBuffer));
				EATEST_VERIFY(rbListString.size() == std::min(s + 1, rbListString.capacity()));
			}
			else if(op == 1)
			{
				// void push_back();
			...
			else if(op == 7)
			{
				...
				EATEST_VERIFY(rbListString.size() == std::min(s + count, rbListString.capacity()));
			}
			else if(op == 8)
			{
				// template <typename InputIterator>
		...
		std::swap(rbListString2, rbListString4);
		EATEST_VERIFY(rbListString2.validate());
		EATEST_VERIFY(rbListString3.validate());
		EATEST_VERIFY(rbListString4.validate());
		...
		RBListString rbListStringM2(std::move(rbListStringM1), RBListString::allocator_type());
...

	{
		...
		typedef std::fixed_vector<float, 8>                     GamepadData_t;
		typedef std::ring_buffer<GamepadData_t>                 GamepadDataDelayBuffer_t;
		typedef std::fixed_vector<GamepadDataDelayBuffer_t, 32> GamepadDataDelayBufferTable_t;
...

	{
		...
		typedef std::ring_buffer<int>		IntBuffer_t;
	...
	{
		{
			...
			std::ring_buffer<int, std::fixed_vector<int, MAX_ELEMENTS, false>> rb(MAX_ELEMENTS - 1);  
		...
		#if !defined(EA_COMPILER_NO_TEMPLATE_ALIASES)
		{
			...
			std::fixed_ring_buffer<int, MAX_ELEMENTS> rb(MAX_ELEMENTS);
		...
		{
			const int MAX_ELEMENTS = 8;
			std::fixed_ring_buffer<int, MAX_ELEMENTS> rb = {0, 1, 2, 3, 4, 5, 6, 7};
...

	{
		...
		struct PaddedVector : public std::vector<int, MallocAllocator>
		{
...

		{
			...
			std::ring_buffer<int, PaddedVector> intBuffer(1);  
			for (int i = 0; i < MAX_ELEMENTS; i++)
