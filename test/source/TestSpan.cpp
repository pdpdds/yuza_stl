...
void TestSpanCtor(int& nErrorCount)
{
	...
	{
		int arr[5] = {0, 1, 2, 3, 4};
		span<int> s(std::begin(arr), 5);
		VERIFY(s.data() == std::begin(arr));
		VERIFY(s.size() == 5);
		VERIFY(!s.empty());
	}
	...
	{
		int arr[5] = {0, 1, 2, 3, 4};
		span<int> s(std::begin(arr), std::end(arr));
		VERIFY(s.data() == std::begin(arr));
		VERIFY(s.size() == 5);
		VERIFY(!s.empty());
	}
...
void TestSpanContainerConversion(int& nErrorCount)
{
...

	{
		...
		VERIFY(s.size() == static_cast<span<int>::index_type>(std::size(v)));
		VERIFY(s.data() == std::data(v));
