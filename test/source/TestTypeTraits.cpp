...
struct NonPod2
{
	...
	struct has_increment_operator : std::false_type {};

	template <typename T>
	struct has_increment_operator<T, std::void_t<decltype(++std::declval<T>())>> : std::true_type {};
#endif
...
template<typename T>
typename std::enable_if<std::is_floating_point<T>::value, T>::type EnableIfTestFunction(T) 
	{ return 999; }
 
template<typename T>
typename std::enable_if<std::is_integral<T>::value, T>::type EnableIfTestFunction(T) 
	{ return 888; }

template<typename T>
typename std::disable_if<std::is_signed<T>::value, T>::type EnableIfTestFunction(T) 
	{ return 777; }
...
// template class std::vector<HiddenAssign>;
...
namespace
{
	const std::string gEmptyStringInstance("");

	const std::integral_constant<int*, nullptr> gIntNullptrConstant;
...
int TestTypeTraits()
{
	...
	static_assert((std::is_assignable<int&, int>::value              == true),   "is_assignable failure");
	static_assert((std::is_assignable<const int&, int>::value        == false),  "is_assignable failure");
	static_assert((std::is_assignable<char*, int*>::value            == false),  "is_assignable failure");
	static_assert((std::is_assignable<char*, const char*>::value     == false),  "is_assignable failure");
	static_assert((std::is_assignable<PodA, PodB*>::value            == false),  "is_assignable failure");
	static_assert((std::is_assignable<Assignable, Pod2>::value       == false),  "is_assignable failure");

	#if EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE
		// These might not succeed unless the implementation is conforming.
		static_assert((std::is_assignable<Assignable, Assignable>::value == true),  "is_assignable failure");
		static_assert((std::is_assignable<Assignable, Pod1>::value       == true),  "is_assignable failure");
		static_assert((std::is_assignable<PodA&, PodA>::value            == true),  "is_assignable failure");

		// These cannot succeed unless the implementation is conforming.
		static_assert((std::is_assignable<void, void>::value             == false),  "is_assignable failure");
		static_assert((std::is_assignable<int, int>::value               == false),  "is_assignable failure");
		static_assert((std::is_assignable<int, const int>::value         == false),  "is_assignable failure");
		static_assert((std::is_assignable<const int, int>::value         == false),  "is_assignable failure");
		static_assert((std::is_assignable<int, int&>::value              == false),  "is_assignable failure");
		static_assert((std::is_assignable<int64_t, int8_t>::value        == false),  "is_assignable failure");
		static_assert((std::is_assignable<bool, bool>::value             == false),  "is_assignable failure");
		static_assert((std::is_assignable<char*, char*>::value           == false),  "is_assignable failure");
		static_assert((std::is_assignable<int, float>::value             == false),  "is_assignable failure");
		static_assert((std::is_assignable<const char*, char*>::value     == false),  "is_assignable failure");
		static_assert((std::is_assignable<int[], int[]>::value           == false),  "is_assignable failure");
	#endif


	// is_lvalue_assignable
	static_assert((std::is_lvalue_assignable<int&, int>::value              == true),   "is_lvalue_assignable failure");
	static_assert((std::is_lvalue_assignable<char*, int*>::value            == false),  "is_lvalue_assignable failure");
	static_assert((std::is_lvalue_assignable<char*, const char*>::value     == false),  "is_lvalue_assignable failure");
	static_assert((std::is_lvalue_assignable<PodA, PodB*>::value            == false),  "is_lvalue_assignable failure");
	static_assert((std::is_lvalue_assignable<Assignable, Pod2>::value       == false),  "is_lvalue_assignable failure");

	#if EASTL_TYPE_TRAIT_is_lvalue_assignable_CONFORMANCE
		// These might not succeed unless the implementation is conforming.
		static_assert((std::is_lvalue_assignable<Assignable, Assignable>::value == true),  "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<Assignable, Pod1>::value       == true),  "is_lvalue_assignable failure");

		// These cannot succeed unless the implementation is conforming.
		static_assert((std::is_lvalue_assignable<void, void>::value             == false),  "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<int, int>::value               == true),   "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<int, const int>::value         == true),   "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<const int, int>::value         == false),  "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<int, int&>::value              == true),   "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<int64_t, int8_t>::value        == true),   "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<bool, bool>::value             == true),   "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<char*, char*>::value           == true),   "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<const char*, char*>::value     == true),   "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<int[], int[]>::value           == false),  "is_lvalue_assignable failure");
		static_assert((std::is_lvalue_assignable<int[3], int[3]>::value         == false),  "is_lvalue_assignable failure"); // Despite that you can memcpy these, C++ syntax doesn't all =-based assignment.
			...
			static_assert((std::is_lvalue_assignable<double, int8_t>::value     == true),   "is_lvalue_assignable failure"); // Sure this might generate a warning, but it's valid syntax.
		#endif
	#endif


	// is_copy_assignable
	static_assert((std::is_copy_assignable<int&>::value              == true),   "is_copy_assignable failure");
	static_assert((std::is_copy_assignable<char>::value              == true),   "is_copy_assignable failure");

	#if EASTL_TYPE_TRAIT_is_assignable_CONFORMANCE
		// These might not succeed unless the implementation is conforming.
		static_assert((std::is_copy_assignable<Assignable>::value    == true),  "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<Assignable>::value    == true),  "is_copy_assignable failure");

		// These cannot succeed unless the implementation is conforming.
		static_assert((std::is_copy_assignable<char*>::value         == true),   "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<PodA>::value          == true),   "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<Assignable>::value    == true),   "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<void>::value          == false),  "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<int>::value           == true),   "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<const int>::value     == false),  "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<int64_t>::value       == true),   "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<bool>::value          == true),   "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<char*>::value         == true),   "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<const char*>::value   == true),   "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<int[3]>::value        == false),  "is_copy_assignable failure");
		static_assert((std::is_copy_assignable<int[]>::value         == false),  "is_copy_assignable failure");
	#endif


	// is_trivially_assignable
	static_assert((std::is_trivially_assignable<int&, int>::value              == true),   "is_trivially_assignable failure");
	static_assert((std::is_trivially_assignable<char*, int*>::value            == false),  "is_trivially_assignable failure");
	static_assert((std::is_trivially_assignable<char*, const char*>::value     == false),  "is_trivially_assignable failure");
	static_assert((std::is_trivially_assignable<PodA, PodB*>::value            == false),  "is_trivially_assignable failure");
	static_assert((std::is_trivially_assignable<Assignable, Assignable>::value == false),  "is_trivially_assignable failure"); // False because not trivial.
	static_assert((std::is_trivially_assignable<Assignable, Pod1>::value       == false),  "is_trivially_assignable failure"); // False because not trivial.
	static_assert((std::is_trivially_assignable<Assignable, Pod2>::value == false),        "is_trivially_assignable failure");
	...
	{  // user reported regression
		...
		static_assert(!std::is_trivially_copyable<Foo>::value, "is_trivially_copyable failure");
	}
	...
	{
		// Test declarations
		std::make_signed<int8_t>::type i8 = -1;
		EATEST_VERIFY(i8 == -1);
		std::make_unsigned<uint8_t>::type u8 = 0xff;
		EATEST_VERIFY(u8 == 0xff);

		std::make_signed<int16_t>::type i16 = -1;
		EATEST_VERIFY(i16 == -1);
		std::make_unsigned<uint16_t>::type u16 = 0xffff;
		EATEST_VERIFY(u16 == 0xffff);

		std::make_signed<int32_t>::type i32 = -1;
		EATEST_VERIFY(i32 == -1);
		std::make_unsigned<uint32_t>::type u32 = 0xffffffff;
		EATEST_VERIFY(u32 == 0xffffffff);

		std::make_signed<int64_t>::type i64 = -1;
		EATEST_VERIFY(i64 == -1);
		std::make_unsigned<uint64_t>::type u64 = UINT64_C(0xffffffffffffffff);
		EATEST_VERIFY(u64 == UINT64_C(0xffffffffffffffff));

		// Test conversions via static_cast:
		u8 = static_cast<std::make_unsigned<uint8_t>::type>(i8);
		EATEST_VERIFY(u8 == 0xff);
		i8 = static_cast<std::make_signed<int8_t>::type>(u8);
		EATEST_VERIFY(i8 == -1);

		u16 = static_cast<std::make_unsigned<uint16_t>::type>(i16);
		EATEST_VERIFY(u16 == 0xffff);
		i16 = static_cast<std::make_signed<int16_t>::type>(u16);
		EATEST_VERIFY(i16 == -1);

		u32 = static_cast<std::make_unsigned<uint32_t>::type>(i32);
		EATEST_VERIFY(u32 == 0xffffffff);
		i32 = static_cast<std::make_signed<int32_t>::type>(u32);
		EATEST_VERIFY(i32 == -1);

		u64 = static_cast<std::make_unsigned<uint64_t>::type>(i64);
		EATEST_VERIFY(u64 == UINT64_C(0xffffffffffffffff));
		i64 = static_cast<std::make_signed<int64_t>::type>(u64);
		EATEST_VERIFY(i64 == -1);
	}
	...
	{
		// To do: Make more thorough tests verifying this. Such tests will probably involve template metaprogramming.
		std::add_const<int32_t>::type i32 = 47;
		EATEST_VERIFY(i32 == 47);

		std::add_volatile<volatile int16_t>::type i16 = 47;
		EATEST_VERIFY(++i16 == 48);

		std::add_cv<const volatile int32_t>::type i64 = 47;
	...
	// as_const
	{
		{
			int i = 42;	
			static_assert(std::is_same<decltype(std::as_const(i)), const int&>::value, "expecting a 'const T&' return type");
			EATEST_VERIFY(std::as_const(i) == 42);
		}

		{
			std::string str = "Electronic Arts";
			static_assert(std::is_same<decltype(std::as_const(str)), const std::string&>::value, "expecting a 'const T&' return type");
			EATEST_VERIFY(std::as_const(str) == "Electronic Arts");
		}
	}
	...
	{
		int x = 17;
		std::add_reference<int>::type xRef = x;
		x++;
		EATEST_VERIFY(xRef == 18);

		std::remove_reference<int&>::type xValue;
		xValue = 3;
		EATEST_VERIFY(xValue == 3);

		std::add_pointer<int>::type xPtr = &x;
		*xPtr = 19;
		EATEST_VERIFY(x == 19);

		std::remove_pointer<int*>::type yValue;
		yValue = 3;
		...
		typedef std::remove_extent<IntArray1>::type Int;
		static_assert((std::is_same<Int, int>::value == true), "remove_extent/is_same failure");

		// remove_all_extents
		typedef int IntArray2[37][54];
		typedef std::remove_all_extents<IntArray2>::type Int2;
		static_assert((std::is_same<Int2, int>::value == true), "remove_all_extents/is_same failure");
	}


	// decay
	{ 
		static_assert((std::is_same<uint32_t,          std::decay<uint32_t>::type>::value                    == true), "is_same failure");
		static_assert((std::is_same<uint32_t,          std::decay<const uint32_t>::type>::value              == true), "is_same failure");
		static_assert((std::is_same<uint32_t,          std::decay<volatile uint32_t>::type>::value           == true), "is_same failure");
		static_assert((std::is_same<uint32_t,          std::decay<uint32_t&>::type>::value                   == true), "is_same failure");
		static_assert((std::is_same<uint32_t,          std::decay<const uint32_t&>::type>::value             == true), "is_same failure");
		static_assert((std::is_same<uint32_t,          std::decay<const volatile uint32_t&>::type>::value    == true), "is_same failure");
	  #if !EASTL_NO_RVALUE_REFERENCES
		static_assert((std::is_same<uint32_t,          std::decay<uint32_t&&>::type>::value                  == true), "is_same failure");
	  #endif
		static_assert((std::is_same<uint32_t*,         std::decay<uint32_t[3]>::type>::value                 == true), "is_same failure");
		static_assert((std::is_same<uint32_t(*)(char), std::decay<uint32_t(char)>::type>::value              == true), "is_same failure");
	}
	...
	#if (EA_ALIGN_MAX_AUTOMATIC >= 64) && defined(EA_PLATFORM_DESKTOP) // Actually there are additional compilers that support alignment of stack-based variables, most significantly clang, GCC 4.4+, and probably others.
	{
		...
		std::aligned_storage<sizeof(Type), kExpectedAlignment>::type data;
		Type* value = new(&data) Type;
		*value = 37;
		EATEST_VERIFY_F((EA::StdC::GetAlignment(value) >= kExpectedAlignment) && (*value == 37), 
						 "std::aligned_storage failure: Expected: %u, Actual: %u", (unsigned)kExpectedAlignment, (unsigned)EA::StdC::GetAlignment(value));

		// Create an array of 100 values aligned.
		std::aligned_storage<sizeof(Type), kExpectedAlignment>::type dataArray[kArraySize];
		...
		EATEST_VERIFY_F((EA::StdC::GetAlignment(valueArray) >= kExpectedAlignment) && (valueArray[0] == 37), 
						 "std::aligned_storage failure: Expected: %u, Actual: %u", (unsigned)kExpectedAlignment, (unsigned)EA::StdC::GetAlignment(valueArray));
	}
	{
		// Test the creation of a single aligned value.
	...
	// is_final
	{
		...
		static_assert(std::is_final<FinalStruct>::value == std::is_final<FinalStruct>::value,  "final struct not correctly detected");
		static_assert(std::is_final<FinalClass>::value  == std::is_final<FinalClass>::value,   "final class not correctly detected");
		static_assert(std::is_final<Enum>::value        == std::is_final<Enum>::value,         "enum not correctly detected");
		static_assert(std::is_final<int>::value         == std::is_final<int>::value,          "int not correctly detected");
		static_assert(std::is_final<Struct>::value      == std::is_final<Struct>::value,       "non-final struct not correctly detected");
		static_assert(std::is_final<Class>::value       == std::is_final<Class>::value,        "non-final class not correctly detected");
    #endif

	// endian (big-endian and little; no mixed-endian/middle-endian)
	static_assert(std::endian::big != std::endian::little, "little-endian and big-endian are not the same");
	static_assert(std::endian::native == std::endian::big || std::endian::native == std::endian::little, "native may be little endian or big endian");
	static_assert(!(std::endian::native == std::endian::big && std::endian::native == std::endian::little), "native cannot be both big and little endian");

	#ifdef EA_SYSTEM_LITTLE_ENDIAN
		static_assert(std::endian::native == std::endian::little,  "must be little endian");
		static_assert(std::endian::native != std::endian::big,     "must not be big endian");
	#else
		static_assert(std::endian::native != std::endian::little,  "must not be little endian");
		static_assert(std::endian::native == std::endian::big,     "must be big endian");
	#endif
	}
