...
// These tell the compiler to compile all the functions for the given class.
	...
	template class std::bitset<1,   uint8_t>;
	template class std::bitset<33,  uint8_t>;
	template class std::bitset<65,  uint8_t>;
	template class std::bitset<129, uint8_t>;
#endif

#if (EASTL_BITSET_WORD_SIZE_DEFAULT != 2)
	template class std::bitset<1,   uint16_t>;
	template class std::bitset<33,  uint16_t>;
	template class std::bitset<65,  uint16_t>;
	template class std::bitset<129, uint16_t>;
#endif

#if (EASTL_BITSET_WORD_SIZE_DEFAULT != 4) // If not already represented 
	template class std::bitset<1,   uint32_t>;
	template class std::bitset<33,  uint32_t>;
	template class std::bitset<65,  uint32_t>;
	template class std::bitset<129, uint32_t>;
#endif

#if (EASTL_BITSET_WORD_SIZE_DEFAULT != 8)
	template class std::bitset<1,   uint64_t>;
	template class std::bitset<33,  uint64_t>;
	template class std::bitset<65,  uint64_t>;
	template class std::bitset<129, uint64_t>;
#endif

#if (EASTL_BITSET_WORD_SIZE_DEFAULT != 16)
	#if EASTL_INT128_SUPPORTED
		template class std::bitset<1,   eastl_uint128_t>;
		template class std::bitset<33,  eastl_uint128_t>;
		template class std::bitset<65,  eastl_uint128_t>;
		template class std::bitset<129, eastl_uint128_t>;
	#endif
#endif
...
int TestBitset()
{
	...
	{
		{
			typedef std::bitset<32, char> bitset_t;
			static_assert(bitset_t::kWordCount == BITSET_WORD_COUNT(bitset_t::kSize, bitset_t::word_type), "bitset failure");
		}
		{
			typedef std::bitset<32, int> bitset_t;
			static_assert(bitset_t::kWordCount == BITSET_WORD_COUNT(bitset_t::kSize, bitset_t::word_type), "bitset failure");
		}
		{
			typedef std::bitset<32, int16_t> bitset_t;
			static_assert(bitset_t::kWordCount == BITSET_WORD_COUNT(bitset_t::kSize, bitset_t::word_type), "bitset failure");
		}
		{
			typedef std::bitset<32, int32_t> bitset_t;
			static_assert(bitset_t::kWordCount == BITSET_WORD_COUNT(bitset_t::kSize, bitset_t::word_type), "bitset failure");
		}
		{
			typedef std::bitset<128, int64_t> bitset_t;
			static_assert(bitset_t::kWordCount == BITSET_WORD_COUNT(bitset_t::kSize, bitset_t::word_type), "bitset failure");
		}
		{
			typedef std::bitset<256, int64_t> bitset_t;
			static_assert(bitset_t::kWordCount == BITSET_WORD_COUNT(bitset_t::kSize, bitset_t::word_type), "bitset failure");
		}
	}
