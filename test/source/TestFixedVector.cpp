...
template class std::fixed_vector<int,        1,  true>;
template class std::fixed_vector<Align64,    1,  true>;
template class std::fixed_vector<TestObject, 1,  true>;

template class std::fixed_vector<int,        1, false>;
template class std::fixed_vector<Align64,    1, false>;
template class std::fixed_vector<TestObject, 1, false>;
...
{
	std::fixed_vector<StructWithContainerOfStructs,4> children;
};
*/
...
int TestFixedVector()
{
	...
	{ // Test the aligned_buffer template
		{
			std::aligned_buffer<sizeof(TestObject), EASTL_ALIGN_OF(TestObject)> toAlignedBuffer;
		...
		{
			std::aligned_buffer<sizeof(Align64), EASTL_ALIGN_OF(Align64)> a64AlignedBuffer;
...

	{
		...
		typedef std::fixed_vector<void*, 160, false> FixedVector;
		FixedVector v;
		...
		EATEST_VERIFY(std::unique(v.begin(), v.end()) == v.end());

		FixedVector::iterator it = std::lower_bound(v.begin(), v.end(), p - 30);
...

	{
		...
		std::fixed_vector<TestObject, 32, false> vTO1;         // <-- bEnableOverflow = false
...

	{
		...
		std::fixed_vector<TestObject, 15, true> vTO2;         // <-- bEnableOverflow = true
		EATEST_VERIFY(TestObject::sTOCount == toCount0);
	...
	{ // Try to repro user report that fixed_vector on the stack crashes.
		std::fixed_vector<int,        10, false> fvif;
		std::fixed_vector<int,        10, true>  fvit;
		std::fixed_vector<TestObject, 10, false> fvof;
		std::fixed_vector<TestObject, 10, true>  fvot;
		std::fixed_vector<int,        10, false, MallocAllocator> fvimf;
		std::fixed_vector<int,        10, true,  MallocAllocator> fvimt;
		std::fixed_vector<TestObject, 10, false, MallocAllocator> fvomf;
		std::fixed_vector<TestObject, 10, true,  MallocAllocator> fvomt;
	...
	{   // Test for crash bug reported by Arpit Baldeva.
		std::fixed_vector<void*, 1, true> test;

		test.push_back(NULL);
		test.push_back(NULL);
		test.erase(std::find(test.begin(), test.end(), (void*)NULL));
		test.erase(std::find(test.begin(), test.end(), (void*)NULL));
...

	{
		...
		fvmv2 = std::move(fvmv1); // Test move assignment operator
		...
		fixed_vector<unique_ptr<unsigned int>, FV_SIZE> fv = std::move(fvmv1); // Test move copy constructor
		for (unsigned int i = 0; i < FV_SIZE; ++i)
		{
			EATEST_VERIFY(!fvmv1[i]);
	...
	{ // Same as above but with custom statefull allocator
		struct MyAlloc : public std::allocator
		{
			MyAlloc()=default;
			MyAlloc(int i) : dummy(i) {}
			int dummy;
		};
		std::fixed_vector<TestObject, 1, true, MyAlloc> fv1;
		fv1.push_back(TestObject(0));
		fv1.push_back(TestObject(0));
		int64_t copyCtorCount0 = TestObject::sTOCopyCtorCount, moveCtorCount0 = TestObject::sTOMoveCtorCount;
		decltype(fv1) fv2(std::move(fv1), MyAlloc(123));
		EATEST_VERIFY(TestObject::sTOCopyCtorCount == copyCtorCount0 && TestObject::sTOMoveCtorCount == (moveCtorCount0 + 2));
	}
	...
	{
		std::fixed_vector<std::variant<int>, 4> v;
		std::fixed_vector<std::variant<int>, 4> b = std::move(v);
	}
	#endif
	return nErrorCount;     
}
