...
template class std::vector<Align64>;
template class std::vector<TestObject>;
...
// the standard utility "std::addressof(T)" which is designed to by-pass user
...
// 		::new(std::addressof(*currentDest)) value_type(value);  // guaranteed to be a pointer
//
...
template class std::vector<HasAddressOfOperator>;  // force compile all functions of vector
...
{
	std::vector<StructWithContainerOfStructs> children;
};
...
struct ScenarioRefEntry
{
	...
	typedef std::vector<RowEntry> RowData;
	RowData Rows;
};
typedef std::vector<ScenarioRefEntry> ScenarRefData;
struct AntMetaDataRecord
{
	ScenarRefData ScenarioRefs;
};
typedef std::vector<AntMetaDataRecord> MetadataRecords;
...
int TestVector()
{
...

	{
		...
		// vector(std::initializer_list<T> ilist, const Allocator& allocator = EASTL_VECTOR_DEFAULT_ALLOCATOR);
		{
			...
			std::vector<float> floatVector{0, 1, 2, 3};
...
// this_type& operator=(std::initializer_list<T> ilist);
...

	{
		...
		std::vector<ItemWithConst> myVec2;
...

	{
		...
		std::vector<TestObject> eastlV(10);
		...
		std::vector<char8_t> eastlVString;
...
// iterator insert(const_iterator position, std::initializer_list<T> ilist);
...

	{
		...
		std::vector<TestObject> toVector3;
...

	{
		...
		// Regression for user-reported possible bug.
		{
			...
			std::vector<int, MallocAllocator> v;
	...
	{  // Test user report that they think they saw code like this leak memory.
		...
		intTest.push_back(1);
		intTest = std::vector<int>();

		std::vector<TestObject> toTest;

		toTest.push_back(TestObject(1));
		toTest = std::vector<TestObject>();
	}
	...
	{  // Regression of user error report for the case of vector<const type>.
		std::vector<int> ctorValues;

		for (int v = 0; v < 10; v++)
			ctorValues.push_back(v);

		std::vector<const ConstType> testStruct(ctorValues.begin(), ctorValues.end());
		std::vector<const int> testInt(ctorValues.begin(), ctorValues.end());
	}

	{  // Regression to verify that const vector works.
		const std::vector<int> constIntVector1;
		EATEST_VERIFY(constIntVector1.empty());

		int intArray[3] = {37, 38, 39};
		const std::vector<int> constIntVector2(intArray, intArray + 3);
		EATEST_VERIFY(constIntVector2.size() == 3);

		const std::vector<int> constIntVector3(4, 37);
		EATEST_VERIFY(constIntVector3.size() == 4);

		const std::vector<int> constIntVector4;
		const std::vector<int> constIntVector5 = constIntVector4;
	}
...
// C++11 cbegin, cend, crbegin, crend
		...
		// const float vector
		const std::vector<float> cFloatVector;
...

	{
		...
		EATEST_VERIFY_MSG(std::find(v_copy.begin(), v_copy.end(), str0) != v_copy.end(), "vector copy string8");
		EATEST_VERIFY_MSG(v.size() == 1, "vector string8 copy size");
		EATEST_VERIFY_MSG(std::find(v.begin(), v.end(), str0) != v.end(), "vector copy string8");
...

	{
		...
		std::vector<int, InstanceAllocator> v0((eastl_size_t)1, (int)0, ia0);
		std::vector<int, InstanceAllocator> v1((eastl_size_t)1, (int)1, ia1);
...

	{
		...
		std::vector<TestMoveAssignToSelf> v1;
...

	{
		...
		std::vector<CopyConstructible> v3(2, CopyConstructible::Create());
		...
		std::vector<CopyConstructible> v4(cbegin(v3), cend(v3));
...

	{
		...
		std::vector<MoveConstructible> v5;
		v5.reserve(2);
...

	{
		...
		std::vector<MoveConstructible> v7(
...

	{
		...
		std::vector<Destructible> v4, v5;
		std::swap(v4, v5);
		EATEST_VERIFY(v4.empty() && v5.empty());

		std::vector<DefaultConstructible> v6(1), v7(2);
		std::swap(v6, v7);
...

	{
		...
		std::vector<MoveAndDefaultConstructible> v8;
		v8.resize(2);
...

	{
		...
		private:
			std::vector<container_value_type> m_vector;
		};
		...
		std::vector<container_value_type> v2(ci.begin(), ci.end()); 
	}
	...
	// If the legacy code path is enabled we cannot handle non-copyable types
		...
		// unique_ptr tests
		{
			// Simple move-assignment test to prevent regressions where std::vector utilizes operations on T that are not necessary.
			{
			...
			{
				// This test verifies that std::vector can handle the move-assignment case where its utilizes two
				...
				InstanceAllocator::reset_all();
				{
					...
					std::vector<std::unique_ptr<int>, InstanceAllocator> v1(a1);
					std::vector<std::unique_ptr<int>, InstanceAllocator> v2(a2);
	...
	{
		// CustomAllocator has no data members which reduces the size of an std::vector via the empty base class optimization.
		typedef std::vector<int, CustomAllocator> EboVector;
		static_assert(sizeof(EboVector) == 3 * sizeof(void*), "");
	}

	// std::erase / std::erase_if tests
	{
		{
			std::vector<int> v = {1, 2, 3, 4, 5, 6, 7, 8, 9};

			std::erase(v, 5);
			VERIFY((v == std::vector<int> {1, 2, 3, 4, 6, 7, 8, 9}));

			std::erase(v, 2);
			VERIFY((v == std::vector<int> {1, 3, 4, 6, 7, 8, 9}));

			std::erase(v, 9);
			VERIFY((v == std::vector<int> {1, 3, 4, 6, 7, 8}));
		}

		{
			std::vector<int> v = {1, 2, 3, 4, 5, 6, 7, 8, 9};
			std::erase_if(v, [](auto i) { return i % 2 == 0; });
			VERIFY((v == std::vector<int>{1, 3, 5, 7, 9}));
		}
	}
