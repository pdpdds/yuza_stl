...
// {
//     std::deque<StructWithContainerOfStructsDefault> children;
// };


///////////////////////////////////////////////////////////////////////////////
typedef std::deque<int>                            EIntDeque;
typedef std::deque<int, EASTLAllocatorType, 1>     EIntDeque1;
typedef std::deque<int, EASTLAllocatorType, 32768> EIntDeque32768;


typedef std::deque<DequeObject>                            EDODeque;
typedef std::deque<DequeObject, EASTLAllocatorType, 1>     EDODeque1;
typedef std::deque<DequeObject, EASTLAllocatorType, 32768> EDODeque32768;
...
int TestDequeComplexMutation()
{
...

	{
		...
		std::advance(itD2NearEnd, d2.size() - 1);
...
int TestDeque()
{
	...
	// test deque support of move-only types
	{
...

		{
			...
			//
			// std::deque<MoveAssignable> d;  // empty deque
			// d.erase(d.begin());
			// EATEST_VERIFY(d.size() == 0);
		}
		...
		// simply test the basic api of deque with a move-only type
		{
			...
			{
				std::deque<MoveAssignable> swapped_d;
...

	{
		...
		// iterator insert(iterator position, std::initializer_list<value_type> ilist);
			...
			std::deque<int>::iterator it = intDeque.insert(intDeque.begin(), { 14, 15 });
	...
	{   // C++11 functionality
		...
		deque<TestObject> toDequeA(std::move(deque3TO33));
		...
		deque<TestObject, MallocAllocator> toDequeB(std::move(deque4TO44), MallocAllocator());
	...
	{   // C++11 functionality
		...
		deque<TestObject, std::allocator, 16> toDequeA;
		...
		deque<TestObject, std::allocator, 16> toDequeC; // Specify a non-small kSubarrayCount of 16 because the move count tests below assume there is no reallocation.
...

	{
		...
		std::deque<int, InstanceAllocator> v0((eastl_size_t)1, (int)0, ia0);
		std::deque<int, InstanceAllocator> v1((eastl_size_t)1, (int)1, ia1);
	...
	{   // Regression of user-reported bug
		...
		std::deque<std::string> testArray;
		std::string s("a");
	...
	{   // Regression of user-reported bug
		...
		// std::deque is missing logic to recenter the pointer array, so it keeps growing 
		...
		std::deque<int, MallocAllocator> x;
		std::deque<int, MallocAllocator> y;
	...
	{ // Regression of user error report for the case of deque<const type>.
		...
		std::deque<const ConstType> testStruct(ctorValues.begin(), ctorValues.end());
		std::deque<const int>       testInt(ctorValues.begin(), ctorValues.end());
	}


	{ // Regression to verify that const deque works.
		const std::deque<int> constIntDeque1;
		VERIFY(constIntDeque1.empty());

		int intArray[3] = { 37, 38, 39 };
		const std::deque<int> constIntDeque2(intArray, intArray + 3);
		VERIFY(constIntDeque2.size() == 3);

		const std::deque<int> constIntDeque3(4, 37);
		VERIFY(constIntDeque3.size() == 4);

		const std::deque<int> constIntDeque4;
		const std::deque<int> constIntDeque5 = constIntDeque4;
	}

	{
		// test shrink_to_fit 
		std::deque<int, CountingAllocator> d(4096);
...

	{
	...
	#endif
		{
			...
			static_assert(std::has_trivial_relocate<a>::value == false, "failure");
	...
	{ // Test erase / erase_if
		{
			...
			std::erase(d, 2);
			VERIFY((d == std::deque<int>{1, 3, 4, 5, 6, 7, 8, 9}));

			std::erase(d, 7);
			VERIFY((d == std::deque<int>{1, 3, 4, 5, 6, 8, 9}));

			std::erase(d, 9);
			VERIFY((d == std::deque<int>{1, 3, 4, 5, 6, 8}));

			std::erase(d, 5);
			VERIFY((d == std::deque<int>{1, 3, 4, 6, 8}));

			std::erase(d, 3);
			VERIFY((d == std::deque<int>{1, 4, 6, 8}));
		}

		{
			std::deque<int> d = {1, 2, 3, 4, 5, 6, 7, 8, 9};
			std::erase_if(d, [](auto i) { return i % 2 == 0; });
			VERIFY((d == std::deque<int>{1, 3, 5, 7, 9}));
		}
	}
