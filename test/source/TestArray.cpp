...
template struct std::array<int>;
template struct std::array<Align32>; // VC++ fails to compile due to error generated by the swap function. C2718: http://msdn.microsoft.com/en-us/library/vstudio/sxe76d9e.aspx
...
int TestArray()
{
...

	{
		...
		// deduction guides
			...
			static_assert(std::is_same_v<decltype(deduced)::value_type, int>, "deduced array value_type mismatch");
	...
	// constexpr tests
	{
		...
		EA_CPP14_CONSTEXPR std::array<int, 4> a = {{ 0, 1, 2, 3 }};

		static_assert(a == std::array<int, 4>{{ 0, 1, 2, 3 }}, "array constexpr failure");
	...
	{
		{
			...
			static_assert(is_same_v<remove_cv_t<decltype(arr)>, std::array<int, 10>>, "unexpected return type");
...

		{
			...
			static_assert(is_same_v<remove_cv_t<decltype(arr)>, std::array<long, 10>>, "unexpected return type");
...

		{
			...
			static_assert(is_same_v<remove_cv_t<decltype(arr)>, std::array<unsigned long, 10>>, "unexpected return type");
...

		{
			...
			static_assert(is_same_v<remove_cv_t<decltype(arr)>, std::array<char, 6>>, "unexpected return type");
