...
int TestHeap()
{
...

	{
			...
			for(int i = 0; (i < 25) && (nErrorCount == 0); i++)
			{
				...
				uint32_t* pArray3           = new uint32_t[nArraySize + 1]; // Array3 is the data in std::make_heap.
...

	{
		...
		std::vector<Align64, CustomAllocator> heap;
		...
		std::remove_heap(heap.begin(), heap.size(), (eastl_size_t)4);
		heap.pop_back();
		EATEST_VERIFY(is_heap(heap.begin(), heap.end()));
...

	{
		...
		std::partial_sort(heap, heap + 3, heap + 5);
	}
