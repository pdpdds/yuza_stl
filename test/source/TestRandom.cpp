...
int TestRandom()
{
...

	{
		...
		{
			std::uniform_int_distribution<uint8_t> uid(1, 6);
			GeneratorUint8 g;
			...
			std::uniform_int_distribution<uint8_t> uid2(1, 6);
			EATEST_VERIFY(uid == uid2);
		}

		{
			std::uniform_int_distribution<uint16_t> uid(1, 6);
			GeneratorUint16 g;
			...
			std::uniform_int_distribution<uint16_t> uid2(1, 6);
			EATEST_VERIFY(uid == uid2);
		}

		{
			...
			std::uniform_int_distribution<uint32_t> uid2(1, 6);
			EATEST_VERIFY(uid == uid2);
		}
	}
...

	{
		...
		std::vector<eastl_size_t> rngArray;
