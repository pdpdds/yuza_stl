...
std::list<TestObject*> toPtrList;

eastl_size_t n1 = intPtrList.size();
eastl_size_t n2 = toPtrList.size();

0042D288  lea         edx,[esp+14h]
0042D28C  call        std::list<TestObject>::size (414180h)
0042D291  push        eax
0042D292  lea         edx,[esp+24h]
0042D296  call        std::list<TestObject>::size (414180h)
```
...
As an example of this, take a look at the implementation of the copy implementation in algorithm. If you are copying an array of scalar values or other trivially copyable values, the compiler will see how the code directs this to the memcpy function and will generate nothing but a memcpy in the final code. For non-memcpyable data types the compiler will automatically understand that in do the right thing.
...
```cpp
std::list<int> gIntList; // Global variable.
...
```cpp
std::list<int>* gIntList = NULL;

void DoSomething()
{
    if(!gIntList) // Or move this to an init function.
        gIntList = new std::list<int>;
...
```cpp
std::list<int> gIntList;

void InitSystem()
{
    new(&gIntList) std::list<int>;
}
...
void VisualizeHashTableBuckets(const HashTable& h)
{
    ...
    for(eastl_size_t i = 0; i < bucketCount; i++)
        largestBucketSize = std::max_alt(largestBucketSize, h.bucket_size(i));
...
> If I had to take a guess, the problem is most likely in the cast to the concrete node type. These are always tricky because, for some strange reason, the debugger is whitespace sensitive with regard to specifying template types. You might try manually checking one of the routines of the specific map instantiation and checking that the placement of whitespace and const within the template expression still matches exactly. In some cases the compiler uses different whitespace rules depending on the value type which makes it impossible to correctly specify a single visualizer – this was the case for std::list<>, for which I was forced to include sections for both cases. The downside is that you have a bunch of (error) entries either way.
...
* Assign a custom allocator to a container. std::fixed_pool provides an implementation.
...
You might consider using std::fixed_pool as such an allocator, as it provides such functionality and allows the user to provide the actual memory used for the pool. Here is some example code:
...
void find_and_replace(String& s, const typename String::value_type* pFind, const typename String::value_type* pReplace)    
{
    for(size_t i; (i = source.find(pFind)) != T::npos; )
        s.replace(i, std::CharStrlen(pFind), pReplace);
}
...
{
    return s.compare(0, std::CharStrlen(p), p) == 0;
}
...
bool ends_with(const String& s, const typename String::value_type* p)
{
    ...
    const typename String::size_type n2 = std::CharStrlen(p);
...
```cpp
typedef std::list<int, fixed_pool_reference> IntList;

IntList::node_type buffer[2];
std::fixed_pool  myPool(buffer, sizeof(buffer), sizeof(Int::node_type), 2);
