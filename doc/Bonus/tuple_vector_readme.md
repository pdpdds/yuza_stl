...
`std::vector<typename T>` has the following counterpart:

	std::fixed_vector<typename T, size_type nodeCount, bool enableOverflow = true>
...
There is a similar counterpart to `std::tuple_vector<typename... Ts>` available as well:

	std::fixed_tuple_vector<size_type nodeCount, bool enableOverflow, typename... Ts>
...
Lastly, `std::vector` and other EASTL data containers support custom Memory Allocator
types, through their template parameters. For example, `std::vector`'s full declaration
is actually:

	std::vector<typename T, typename AllocatorType = EASTLAllocatorType>
...
definition:
	...
	std::tuple_vector_alloc<typename AllocatorType, typename... Ts>
...
algorithms run against `std::tuple_vector` and `std::vector`, such as
...
running `std::sort` on the data containers. More information about the
...
the iteration case, which runs `std::find_if`. This
...
in order to manage the various elements, or interact with `std::tuple` through
