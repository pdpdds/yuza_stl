...
typedef std::deque<ValuePair, EASTLAllocatorType, 128> EaDeque;  // What value do we pick for the subarray size to make the comparison fair? Using the default isn't ideal because it results in this test measuring speed efficiency and ignoring memory efficiency. 
...
namespace
{
	...
	template <typename Container>
	void TestPushFront(EA::StdC::Stopwatch& stopwatch, Container& c, std::vector<uint32_t>& intVector)
	{
		stopwatch.Restart();
		for(eastl_size_t j = 0, jEnd = intVector.size(); j < jEnd; j++)
		{
	...
	void TestFind(EA::StdC::Stopwatch& stopwatch, Container& c)
	{
		...
		stopwatch.Restart();
		typename Container::iterator it = std::find(c.begin(), c.end(), vp);
		stopwatch.Stop();
		if(it != c.end())
			sprintf(Benchmark::gScratchBuffer, "%u", (unsigned)(*it).key);
	}
	...
	void TestSort(EA::StdC::Stopwatch& stopwatch, Container& c)
	{
		...
		VPCompare vpCompare;
		stopwatch.Restart();
		std::quick_sort(c.begin(), c.end(), vpCompare);
		stopwatch.Stop();
