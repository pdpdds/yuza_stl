...
typedef std::vector<ValuePair>  EaVectorVP;

typedef std::vector<uint32_t>     StdVectorInt;
typedef std::vector<uint32_t>   EaVectorInt;
...
namespace
{
	...
	{
		stopwatch.Restart();
		std::quick_sort(eaVectorVP.begin(), eaVectorVP.end());
		stopwatch.Stop();
	...
	{
		stopwatch.Restart();
		std::quick_sort(eaVectorInt.begin(), eaVectorInt.end());
		stopwatch.Stop();
	...
	{
		stopwatch.Restart();
		std::quick_sort(eaVectorTO.begin(), eaVectorTO.end());
		stopwatch.Stop();
...
namespace
{
	...
	{
		sf_qsort,             // C qsort
		sf_shell_sort,        // std::shell_sort.
		sf_heap_sort,         // std::heap_sort
		sf_merge_sort,        // std::merge_sort
		sf_merge_sort_buffer, // std::merge_sort_buffer
		sf_comb_sort,         // std::comb_sort
		sf_bubble_sort,       // std::bubble_sort
		sf_selection_sort,    // std::selection_sort
		sf_shaker_sort,       // std::shaker_sort
		sf_quick_sort,        // std::quick_sort
		sf_tim_sort,          // std::tim_sort
		sf_insertion_sort,    // std::insertion_sort
		sf_std_sort,          // std::sort
		sf_std_stable_sort,   // std::stable_sort
		sf_radix_sort,        // std::radix_sort (unconventional sort)
		sf_count              //
	};
	...
	const char* GetSortFunctionName(int sortFunctionType)
	{
		switch (sortFunctionType)
		{
			...
			case sf_insertion_sort:
				return "std::insertion_sort";

			case sf_shell_sort:
				return "std::shell_sort";

			case sf_heap_sort:
				return "std::heap_sort";

			case sf_merge_sort:
				return "std::merge_sort";

			case sf_merge_sort_buffer:
				return "std::merge_sort_buffer";

			case sf_comb_sort:
				return "std::comb_sort";

			case sf_bubble_sort:
				return "std::bubble_sort";

			case sf_selection_sort:
				return "std::selection_sort";

			case sf_shaker_sort:
				return "std::shaker_sort";

			case sf_radix_sort:
				return "std::radix_sort";
	...
	void Randomize(std::vector<ElementType>& v, EA::UnitTest::RandGenT<RandomType>& rng, RandomizationType type)
	{
...
int CompareSortPerformance()
{
	...
	const eastl_size_t kArraySizeMax = *std::max_element(std::begin(kSizes), std::end(kSizes));
	...
	std::vector<SortFunctionType> allSortFunctions;
...

	{
		...
		typedef std::less<ElementType>      CompareFunction;
				...
				for (size_t sizeType = 0; sizeType < EAArrayCount(kSizes); sizeType++)
				{
					...
					for (SortFunctionType sortFunction : sortFunctions)
					{
						...
						switch (sortFunction)
						{
							case sf_quick_sort:
								stopwatch.Restart();
								std::quick_sort(v.begin(), v.end(), CompareFunction());
								stopwatch.Stop();
								break;

							case sf_tim_sort:
								stopwatch.Restart();
								std::tim_sort_buffer(v.begin(), v.end(), pBuffer, CompareFunction());
								stopwatch.Stop();
								break;

							case sf_insertion_sort:
								stopwatch.Restart();
								std::insertion_sort(v.begin(), v.end(), CompareFunction());
								stopwatch.Stop();
								break;

							case sf_shell_sort:
								stopwatch.Restart();
								std::shell_sort(v.begin(), v.end(), CompareFunction());
								stopwatch.Stop();
								break;

							case sf_heap_sort:
								stopwatch.Restart();
								std::heap_sort(v.begin(), v.end(), CompareFunction());
								stopwatch.Stop();
								break;

							case sf_merge_sort:
								stopwatch.Restart();
								std::merge_sort(v.begin(), v.end(), *get_default_allocator((EASTLAllocatorType*)NULL), CompareFunction());
								stopwatch.Stop();
								break;

							case sf_merge_sort_buffer:
								stopwatch.Restart();
								std::merge_sort_buffer(v.begin(), v.end(), pBuffer, CompareFunction());
								stopwatch.Stop();
								break;

							case sf_comb_sort:
								stopwatch.Restart();
								std::comb_sort(v.begin(), v.end(), CompareFunction());
								stopwatch.Stop();
								break;

							case sf_bubble_sort:
								stopwatch.Restart();
								std::bubble_sort(v.begin(), v.end(), CompareFunction());
								stopwatch.Stop();
								break;

							case sf_selection_sort:
								stopwatch.Restart();
								std::selection_sort(v.begin(), v.end(), CompareFunction());
								stopwatch.Stop();
								break;

							case sf_shaker_sort:
								stopwatch.Restart();
								std::shaker_sort(v.begin(), v.end(), CompareFunction());
								stopwatch.Stop();
								break;

							case sf_radix_sort:
								stopwatch.Restart();
								std::radix_sort<ElementType*, identity_extract_radix_key<ElementType>>(v.begin(), v.end(), pBuffer);
								stopwatch.Stop();
								break;
...

	{
		...
		sortFunctions.erase(std::remove(sortFunctions.begin(), sortFunctions.end(), sf_radix_sort), sortFunctions.end());
...

	{
		...
		sortFunctions.erase(std::remove(sortFunctions.begin(), sortFunctions.end(), sf_qsort), sortFunctions.end());
				...
				for (size_t sizeType = 0; sizeType < EAArrayCount(kSizes); sizeType++)
				{
					...
					for (SortFunctionType sortFunction : sortFunctions)
					{
						...
						switch (sortFunction)
						{
							...
							case sf_radix_sort:
								stopwatch.Restart();
								std::radix_sort<ElementType*, slow_assign_extract_radix_key<ElementType>>(v.begin(), v.end(), pBuffer);
								stopwatch.Stop();
								break;
...
typedef std::function<void(std::string &output, const char* sortFunction, const char* randomizationType, size_t size, size_t numSubArrays, const BenchmarkResult &result)> OutputResultCallback;
typedef std::function<void(BenchmarkResult &result)> PostExecuteCallback;
typedef std::function<void()> PreExecuteCallback;


template<class ElementType, class CompareFunction>
static int CompareSmallInputSortPerformanceHelper(std::vector<eastl_size_t> &arraySizes, std::vector<SortFunctionType> &sortFunctions, const PreExecuteCallback &preExecuteCallback, const PostExecuteCallback &postExecuteCallback, const OutputResultCallback &outputResultCallback)
{
	int nErrorCount = 0;
	...
	EA::StdC::Stopwatch stopwatch(EA::StdC::Stopwatch::kUnitsCPUCycles);
	EA::StdC::Stopwatch stopwatchGlobal(EA::StdC::Stopwatch::kUnitsSeconds);
	const eastl_size_t kArraySizeMax = *std::max_element(std::begin(arraySizes), std::end(arraySizes));
	const int kRunCount = 4;
	...
	for (int i = 0; i < kRandomizationTypeCount; i++)
	{
		for (size_t size : arraySizes)
		{
			for (SortFunctionType sortFunction : sortFunctions)
			{
				...
				for (int c = 0; c < kRunCount; c++)
				{
					std::vector<ElementType> v(size * numSubArrays);
					...
					switch (sortFunction)
					{
						case sf_quick_sort:
							...
							{
								std::quick_sort(begin, begin + size, CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_tim_sort:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::tim_sort_buffer(begin, begin + size, pBuffer, CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_insertion_sort:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::insertion_sort(begin, begin + size, CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_shell_sort:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::shell_sort(begin, begin + size, CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_heap_sort:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::heap_sort(begin, begin + size, CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_merge_sort:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::merge_sort(begin, begin + size, *get_default_allocator((EASTLAllocatorType*)NULL), CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_merge_sort_buffer:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::merge_sort_buffer(begin, begin + size, pBuffer, CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_comb_sort:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::comb_sort(begin, begin + size, CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_bubble_sort:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::bubble_sort(begin, begin + size, CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_selection_sort:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::selection_sort(begin, begin + size, CompareFunction());
							}
							stopwatch.Stop();
							break;
						...
						case sf_shaker_sort:
							...
							for (auto begin = v.begin(); begin != v.end(); begin += size)
							{
								std::shaker_sort(begin, begin + size, CompareFunction());
							}
							stopwatch.Stop();
							break;
					...
					for (auto begin = v.begin(); begin != v.end(); begin += size)
					{
						VERIFY(std::is_sorted(begin, begin + size));
					}
				} // for each run
...
{
	int nErrorCount = 0;
	std::vector<eastl_size_t> arraySizes{1, 2, 3, 4, 7, 8, 15, 16, 31, 32, 64, 128, 256};
	...
	std::vector<SortFunctionType> sortFunctions{sf_quick_sort,     sf_merge_sort_buffer,     sf_bubble_sort, sf_comb_sort,
	...
	nErrorCount += CompareSmallInputSortPerformanceHelper<uint32_t, std::less<uint32_t>>(
	    arraySizes, sortFunctions, PreExecuteCallback([]() {}), PostExecuteCallback([](BenchmarkResult&) {}),
	    OutputResultCallback([](std::string& output, const char* sortFunction, const char* randomizationType,
	...
	nErrorCount += CompareSmallInputSortPerformanceHelper<SlowAssign<uint32_t>, std::less<SlowAssign<uint32_t>>>(
...
void BenchmarkSort()
{
	...
	{
		std::vector<uint32_t> intVector(10000);
