...
typedef std::vector<unsigned char> EaVectorUChar;

typedef std::vector<signed char>     StdVectorSChar;
typedef std::vector<signed char>   EaVectorSChar;

typedef std::vector<uint32_t>        StdVectorUint32;
typedef std::vector<uint32_t>      EaVectorUint32;

typedef std::vector<uint64_t>        StdVectorUint64;
typedef std::vector<uint64_t>      EaVectorUint64;

typedef std::vector<TestObject>      StdVectorTO;
typedef std::vector<TestObject>    EaVectorTO;
...
namespace
{
	...
	void TestFindEndEa(EA::StdC::Stopwatch& stopwatch, const std::string& sTest, const char* pSearchStringBegin, const char* pSearchStringEnd)
	{
		stopwatch.Restart();
		std::string::const_iterator it = std::find_end(sTest.begin(), sTest.end(), pSearchStringBegin, pSearchStringEnd);
		stopwatch.Stop();
	...
	void TestSearchEa(EA::StdC::Stopwatch& stopwatch, const std::string& sTest, const char* pSearchStringBegin, const char* pSearchStringEnd)
	{
		stopwatch.Restart();
		std::string::const_iterator it = std::search(sTest.begin(), sTest.end(), pSearchStringBegin, pSearchStringEnd);
		stopwatch.Stop();
	...
	void TestSearchNEa(EA::StdC::Stopwatch& stopwatch, const std::string& sTest, int n, char c)
	{
		stopwatch.Restart();
		std::string::const_iterator it = std::search_n(sTest.begin(), sTest.end(), n, c);
		stopwatch.Stop();
...
{
	{
		...
		std::vector<uint32_t> eaVectorUint321(10000);
		std::vector<uint32_t> eaVectorUint322(10000);

		std::vector<uint64_t>   stdVectorUint64(100000);
		std::vector<uint64_t> eaVectorUint64(100000);
...
{
	{
		...
		std::vector<void*>    eaVectorVoid(100000);

		std::vector<char>       stdVectorChar(100000);
		std::vector<char>     eaVectorChar(100000);

		std::vector<bool>       stdVectorBool(100000);
		std::vector<bool>     eaVectorBool(100000);
...
void BenchmarkAlgorithm6(EASTLTest_Rand& /*rng*/, EA::StdC::Stopwatch& stopwatch1, EA::StdC::Stopwatch& stopwatch2)
{
	...
	std::vector<LargePOD>* peaVectorLP1  = new std::vector<LargePOD>(100);
	std::vector<LargePOD>* peaVectorLP2  = new std::vector<LargePOD>(100);

	// Aliases.
	std::vector<LargePOD>&   stdVectorLP1 = *pstdVectorLP1;
	std::vector<LargePOD>&   stdVectorLP2 = *pstdVectorLP2;
	std::vector<LargePOD>& eaVectorLP1  = *peaVectorLP1;
	std::vector<LargePOD>& eaVectorLP2  = *peaVectorLP2;
...
{
	{
		...
		std::list<TestObject>   eaListTO(10000);

		std::vector<TestObject>   stdVectorTO(10000);
		std::vector<TestObject> eaVectorTO(10000);
...

	{
		...
		std::slist<int32_t> eaSlistIntLarge(10000);
		std::generate(eaSlistIntLarge.begin(), eaSlistIntLarge.end(), GenerateIncrementalIntegers<int32_t>());
		...
		std::vector< SizedPOD<32> > eaVectorLargePod32(10000);
		for(std::vector< SizedPOD<32> >::iterator it = eaVectorLargePod32.begin(); it != eaVectorLargePod32.end(); ++it)
			memset(&*it, 0, sizeof(SizedPOD<32>));

		std::list<int32_t> stdListIntLarge(10000);
		std::generate(stdListIntLarge.begin(), stdListIntLarge.end(), GenerateIncrementalIntegers<int32_t>());

		std::list<int32_t> eaListIntLarge(10000);
		std::generate(eaListIntLarge.begin(), eaListIntLarge.end(), GenerateIncrementalIntegers<int32_t>());


		std::vector<int32_t> stdVectorIntLarge(10000);
		std::generate(stdVectorIntLarge.begin(), stdVectorIntLarge.end(), GenerateIncrementalIntegers<int32_t>());

		std::vector<int32_t> eaVectorIntLarge(10000);
		std::generate(eaVectorIntLarge.begin(), eaVectorIntLarge.end(), GenerateIncrementalIntegers<int32_t>());


		std::list<int32_t> stdListIntSmall(10);
		std::generate(stdListIntLarge.begin(), stdListIntLarge.end(), GenerateIncrementalIntegers<int32_t>());

		std::list<int32_t> eaListIntSmall(10);
		std::generate(eaListIntLarge.begin(), eaListIntLarge.end(), GenerateIncrementalIntegers<int32_t>());
		...
		std::vector<int32_t> eaVectorIntSmall(10);
		std::generate(eaVectorIntLarge.begin(), eaVectorIntLarge.end(), GenerateIncrementalIntegers<int32_t>());



		std::list<TestObject> stdListTOLarge(10000);
		std::generate(stdListTOLarge.begin(), stdListTOLarge.end(), GenerateIncrementalIntegers<TestObject>());

		std::list<TestObject> eaListTOLarge(10000);
		std::generate(eaListTOLarge.begin(), eaListTOLarge.end(), GenerateIncrementalIntegers<TestObject>());


		std::vector<TestObject> stdVectorTOLarge(10000);
		std::generate(stdVectorTOLarge.begin(), stdVectorTOLarge.end(), GenerateIncrementalIntegers<TestObject>());

		std::vector<TestObject> eaVectorTOLarge(10000);
		std::generate(eaVectorTOLarge.begin(), eaVectorTOLarge.end(), GenerateIncrementalIntegers<TestObject>());


		std::list<TestObject> stdListTOSmall(10);
		std::generate(stdListTOSmall.begin(), stdListTOSmall.end(), GenerateIncrementalIntegers<TestObject>());

		std::list<TestObject> eaListTOSmall(10);
		std::generate(eaListTOSmall.begin(), eaListTOSmall.end(), GenerateIncrementalIntegers<TestObject>());


		std::vector<TestObject> stdVectorTOSmall(10);
		std::generate(stdVectorTOSmall.begin(), stdVectorTOSmall.end(), GenerateIncrementalIntegers<TestObject>());

		std::vector<TestObject> eaVectorTOSmall(10);
		std::generate(eaVectorTOSmall.begin(), eaVectorTOSmall.end(), GenerateIncrementalIntegers<TestObject>());
		...
		for(int i = 0; i < 2; i++)
		{
			...
			TestRotateEa (stopwatch2,  eaSlistIntLarge.begin(), std::next( eaSlistIntLarge.begin(),  (eaSlistIntLarge.size() / 2) - 1),  eaSlistIntLarge.end());
			if(i == 1)
			...
			TestRotateEa (stopwatch2,  eaVectorLargePod32.begin(), std::next( eaVectorLargePod32.begin(),  (eaVectorLargePod32.size() / 2) - 1),  eaVectorLargePod32.end());
			if(i == 1)
			...
			TestRotateEa (stopwatch2,  eaListIntLarge.begin(), std::next( eaListIntLarge.begin(),  (eaListIntLarge.size() / 2) - 1),  eaListIntLarge.end());
			if(i == 1)
			...
			TestRotateEa (stopwatch2,  eaVectorIntLarge.begin(), std::next( eaVectorIntLarge.begin(),  (eaVectorIntLarge.size() / 2) - 1),  eaVectorIntLarge.end());
			if(i == 1)
			...
			TestRotateEa (stopwatch2,  eaListIntSmall.begin(), std::next( eaListIntSmall.begin(),  (eaListIntSmall.size() / 2) - 1),  eaListIntSmall.end());
			if(i == 1)
			...
			TestRotateEa (stopwatch2,  eaVectorIntSmall.begin(), std::next( eaVectorIntSmall.begin(),  (eaVectorIntSmall.size() / 2) - 1),  eaVectorIntSmall.end());
			if(i == 1)
			...
			TestRotateEa (stopwatch2,  eaListTOLarge.begin(), std::next( eaListTOLarge.begin(),  (eaListTOLarge.size() / 2) - 1),  eaListTOLarge.end());
			if(i == 1)
			...
			TestRotateEa (stopwatch2,  eaVectorTOLarge.begin(), std::next( eaVectorTOLarge.begin(),  (eaVectorTOLarge.size() / 2) - 1),  eaVectorTOLarge.end());
			if(i == 1)
			...
			TestRotateEa (stopwatch2,  eaListTOSmall.begin(), std::next( eaListTOSmall.begin(),  (eaListTOSmall.size() / 2) - 1),  eaListTOSmall.end());
			if(i == 1)
			...
			TestRotateEa (stopwatch2,  eaVectorTOSmall.begin(), std::next( eaVectorTOSmall.begin(),  (eaVectorTOSmall.size() / 2) - 1),  eaVectorTOSmall.end());
			if(i == 1)
...
void BenchmarkAlgorithm8(EASTLTest_Rand& rng, EA::StdC::Stopwatch& stopwatch1, EA::StdC::Stopwatch& stopwatch2)
{
	...
	std::vector<int> srcVecA(ElementCount);
	std::vector<int> srcVecB(ElementCount);
	...
	std::vector<int> eaVecAInt(ElementCount);
	std::vector<int> eaVecBInt(ElementCount);
	std::vector<int> eaVecOutInt(2 * ElementCount);
	std::vector<TestObject> eaVecATestObject(ElementCount);
	std::vector<TestObject> eaVecBTestObject(ElementCount);
	std::vector<TestObject> eaVecOutTestObject(2 * ElementCount);
	...
	for (int pattern = 0; pattern < P_Count; pattern++)
	{
		if (pattern == P_Random)
		{
			std::generate(srcVecA.begin(), srcVecA.end(), [&]{ return int(rng()); });
			std::sort(srcVecA.begin(), srcVecA.end());
			std::generate(srcVecB.begin(), srcVecB.end(), [&] { return int(rng()); });
			std::sort(srcVecB.begin(), srcVecB.end());
		}
		...
		for (int i = 0; i < 2; i++)
		{
			std::copy(srcVecA.begin(), srcVecA.end(), stdVecAInt.begin());
			std::copy(srcVecB.begin(), srcVecB.end(), stdVecBInt.begin());
			std::copy(srcVecA.begin(), srcVecA.end(), eaVecAInt.begin());
			std::copy(srcVecB.begin(), srcVecB.end(), eaVecBInt.begin());
