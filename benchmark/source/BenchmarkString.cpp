...
void BenchmarkString()
{
...

	{
		for(int i = 0; i < 2; i++)
		{
			...
			std::basic_string<char8_t>  es8(16, 0);   // different tradeoffs related to startup size. Initial operations are faster
			...
			std::basic_string<char16_t> es16(16, 0);  // the same baseline allocation.
			...
			std::basic_string<char8_t>  es8X(es8);
			std::basic_string<char16_t>   ss16X(ss16);
			std::basic_string<char16_t> es16X(es16);
