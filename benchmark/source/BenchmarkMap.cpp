...
typedef std::map<TestObject, uint32_t>   EaMapTOUint32;
...
void BenchmarkMap()
{
	...
	{
		std::vector< std::pair<TestObject, uint32_t> >   stdVector(10000);
		std::vector< std::pair<TestObject, uint32_t> > eaVector(10000);
		...
		for(eastl_size_t i = 0, iEnd = stdVector.size(); i < iEnd; i++)
		{
			...
			eaVector[i]  = std::pair<TestObject, uint32_t>(TestObject(n1), n2);
		}
		...
		for(int i = 0; i < 2; i++)
		{
			...
			const std::pair<TestObject, uint32_t> eaHighValue(TestObject(0x7fffffff), 0x7fffffff);
