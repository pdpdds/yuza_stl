...
#include <EASTL/bonus/tuple_vector>
#include <EASTL/sort>
...
typedef std::tuple_vector<uint64_t>   EaTupleVectorUint64;
...
typedef std::tuple<uint64_t, PaddingStruct> PaddedTuple;
typedef std::vector<PaddedTuple> StdVectorUint64Padded;
typedef std::tuple_vector<uint64_t, PaddingStruct> EaTupleVectorUint64Padded;
...
namespace 
{
	...
	{
		std::tuple<uint64_t> val(0xffffffffffff);
		stopwatch.Restart();
		EaTupleVectorUint64::iterator it = std::find(c.begin(), c.end(), val);
		stopwatch.Stop();
		if (it != c.end())
			sprintf(Benchmark::gScratchBuffer, "%u", (unsigned)std::get<0>(*it));
	}
	...
	void TestSort(EA::StdC::Stopwatch& stopwatch, EaTupleVectorUint64& c)
	{
		...
		std::quick_sort(c.begin(), c.end());
		stopwatch.Stop();
		sprintf(Benchmark::gScratchBuffer, "%u", (unsigned)(std::get<0>(c[0]) & 0xffffffff));
	}
	...
	template <typename Container>
	void TestTuplePushBack(EA::StdC::Stopwatch& stopwatch, Container& c, std::vector<uint32_t>& intVector)
	{
		stopwatch.Restart();
		for (eastl_size_t j = 0, jEnd = intVector.size(); j < jEnd; j++)
		{
	...
	void TestTupleFind(EA::StdC::Stopwatch& stopwatch, Container& c)
	{
		stopwatch.Restart();
		typedef typename Container::iterator iterator_t; // This typedef is required to get this code to compile on RVCT
		iterator_t it = std::find_if(c.begin(), c.end(), [](auto tup) { return std::get<0>(tup) == 0xFFFFFFFF; });
		stopwatch.Stop();
		if (it != c.end())
			sprintf(Benchmark::gScratchBuffer, "%u", (unsigned)std::get<0>(*it));
	}
	...
	void TestTupleSort(EA::StdC::Stopwatch& stopwatch, Container& c)
	{
		...
		// vector access speed and not be polluted by sort speed.
		stopwatch.Restart();
		std::quick_sort(c.begin(), c.end(), [](auto a, auto b) { return std::get<0>(a) < std::get<0>(b); });
		stopwatch.Stop();
		sprintf(Benchmark::gScratchBuffer, "%u", (unsigned)(std::get<0>(c[0]) & 0xffffffff));
	}
...
void BenchmarkTupleVector()
{
...

	{
		...
		for(int i = 0; i < 2; i++)
		{
			...
			std::tuple_vector<MovableType> eaTupleVectorMovableType;
			...
			std::tuple_vector<AutoRefCount<RefCounted> > eaTupleVectorAutoRefCount;
