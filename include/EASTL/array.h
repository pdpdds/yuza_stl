...
namespace std
{
	...
	{
		static_assert(std::is_constructible_v<T, T&>, "element type T must be copy-initializable");
		static_assert(!std::is_array_v<T>, "passing multidimensional arrays to to_array is ill-formed");
	...
	EA_CONSTEXPR std::array<std::remove_cv_t<T>, N> to_array(T (&&a)[N])
	{
		static_assert(std::is_move_constructible_v<T>, "element type T must be move-constructible");
		static_assert(!std::is_array_v<T>, "passing multidimensional arrays to to_array is ill-formed");
