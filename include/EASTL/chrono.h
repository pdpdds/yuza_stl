...
#if defined(EA_PLATFORM_MICROSOFT) && !defined(EA_PLATFORM_MINGW) && !defined(SKYOS32)
	// Nothing to do
...
namespace chrono
{
	...
	namespace Internal
	{
		...
		{
		#if defined EA_PLATFORM_MICROSOFT  && !defined(SKYOS32)
			auto queryFrequency = []
			{
		   ...
		   return mach_absolute_time();
			...
			#endif
		#elif defined(EA_PLATFORM_SKYOS32)
			struct timeval tv;
			gettimeofday(&tv, NULL);
			const uint64_t nMicroseconds = (uint64_t)tv.tv_usec + ((uint64_t)tv.tv_sec * 1000000);
			return nMicroseconds;
