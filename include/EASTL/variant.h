...
namespace std
{
	namespace internal
	{
		...
		// We can remove these utilities when C++17 'constexpr if' is available.
		//
		template<typename T, bool = std::is_destructible_v<T>>
	...
	public:
		...
		EA_CPP14_CONSTEXPR explicit variant(in_place_type_t<T>, Args&&... args)
			: variant(in_place<meta::get_type_index_v<T, Types...>>, std::forward<Args>(args)...)
		{}
		...
		EA_CPP14_CONSTEXPR explicit variant(in_place_type_t<T>, std::initializer_list<U> il, Args&&... args)
		    : variant(in_place<meta::get_type_index_v<T, Types...>>, il, std::forward<Args>(args)...)
		{}
	...
	struct visitor_caller
	{
		                                             ...
		                                             Variants&&... variants)
		{
			...
			return visitor_caller<Visitor, Variants...>::call(
				...
				index_sequence<ArrayIndices...>(),
				std::make_tuple(get<ArgsIndices>(std::forward<ArgsTuple>(args))..., get<I>(std::forward<Variant>(variant))),
	...
	struct visitor_caller<Visitor, Variant>
	{
		...
		static decltype(auto) EA_CONSTEXPR invoke_visitor(Visitor&& visitor, index_sequence<ArgsIndices...>, ArgsTuple&& args, Variant&& variant)
		{
			return static_cast<R>(invoke(
				std::forward<Visitor>(visitor),
				get<ArgsIndices>(std::forward<ArgsTuple>(args))...,
				get<I>(std::forward<Variant>(variant))
				));
		}
		...
		static decltype(auto) EA_CPP14_CONSTEXPR call(Visitor&& visitor, index_sequence<ArgsIndices...>, index_sequence<ArrayIndices...>, ArgsTuple&& args, Variant&& variant)
		{
			...
			return callers[std::forward<Variant>(variant).index()](
				std::forward<Visitor>(visitor),
				index_sequence<ArgsIndices...>(),
				std::forward<ArgsTuple>(args),
				std::forward<Variant>(variant)
	...
	namespace internal
	{
		...
		{
			template <typename A, typename B, typename = std::enable_if_t<std::is_same_v<std::decay_t<A>, std::decay_t<B>>>>
			...
			{
				EASTL_ASSERT_MSG(false, "std::variant<> comparison function called on two different types at different indices! This is a library bug! Please file bug report.");
				return false;
			}
		};
