...
#define EASTL_COMPILER_ATOMIC_LOAD_READ_DEPENDS_N(type, ret, ptr)		\
	{																	\
		static_assert(std::is_pointer_v<type>, "std::atomic<T> : Read Depends Type must be a Pointer Type!"); \
		static_assert(std::is_pointer_v<std::remove_pointer_t<decltype(ptr)>>, "std::atomic<T> : Read Depends Ptr must be a Pointer to a Pointer!"); \
																		\
