...
namespace internal
{
	...
	{
		static EASTL_CPP17_INLINE_VARIABLE constexpr bool value = std::internal::is_atomic_lockfree_size<T>::value;
	};


	template <typename T>
	using select_atomic_inherit_0 = typename std::conditional<std::is_same_v<bool, T> || std::internal::is_user_type_suitable_for_primary_template<T>::value,
...
#define EASTL_ATOMIC_CLASS_IMPL(type, base, valueType, differenceType)	\
	...
	public:																\
																		\
		static EASTL_CPP17_INLINE_VARIABLE constexpr bool is_always_lock_free = std::internal::is_atomic_lockfree_size<type>::value; \
																		\
	...
	public: /* ctors */													\
		...
		{																\
																		...
																		\
		EA_CONSTEXPR atomic() EA_NOEXCEPT_IF(std::is_nothrow_default_constructible_v<type>) = default; \
																		\
	public:																\
																		\
...
template <typename T>
struct atomic<T, std::enable_if_t<std::is_integral_v<T> && !std::is_same_v<bool, T>>> : protected std::internal::atomic_integral_width<T>
{
