...
namespace std
{
	...
	{
		return DoInsertValue(has_unique_keys_type(), piecewise_construct, std::forward_as_tuple(key),
	...
	{
		return DoInsertValue(has_unique_keys_type(), piecewise_construct, std::forward_as_tuple(std::move(key)),
		                     std::forward_as_tuple(std::forward<Args>(args)...));
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::try_emplace(const_iterator, const key_type& key, Args&&... args)
	{
		insert_return_type result = DoInsertValue(
		    ...
		    value_type(piecewise_construct, std::forward_as_tuple(key), std::forward_as_tuple(std::forward<Args>(args)...)));
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::try_emplace(const_iterator, key_type&& key, Args&&... args)
	{
		insert_return_type result =
		    DoInsertValue(has_unique_keys_type(), value_type(piecewise_construct, std::forward_as_tuple(std::move(key)),
		                                                     std::forward_as_tuple(std::forward<Args>(args)...)));
